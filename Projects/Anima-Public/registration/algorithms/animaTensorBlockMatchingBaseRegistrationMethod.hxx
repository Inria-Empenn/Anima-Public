...
TensorBlockMatchingBaseRegistrationMethod<TInputImage>
::TensorBlockMatchingBaseRegistrationMethod()
{
    m_Abort = false;
    ...
    m_BlockTensorVarianceThreshold = 0.000001;
...
template <typename TInputImage>
typename TensorBlockMatchingBaseRegistrationMethod<TInputImage>::BaseInputTransformType::Pointer
TensorBlockMatchingBaseRegistrationMethod<TInputImage>
...
{
    typedef anima::FiniteStrainTensorResampleImageFilter <typename InputImageType::InternalPixelType, InputImageType::ImageDimension,
    ...
    resample->SetOutputLargestPossibleRegion(m_FixedImage->GetLargestPossibleRegion());
    ...
    resampleFixed->SetOutputLargestPossibleRegion(m_FixedImage->GetLargestPossibleRegion());
...

{
    ...
    switch (m_MetricKind)
    {
        ...
        case TensorOrientedGeneralizedCorrelation:
        {
            ...
            tmpMetric->SetOrientationPenalty(m_MetricKind == TensorOrientedGeneralizedCorrelation);
            tmpMetric->SetVarianceThreshold(this->GetBlockTensorVarianceThreshold());
        ...
        default:
            std::cerr << "Tensor based metric not implemented yet" << std::endl;
            break;
    }

    metric->SetRotateTensors(m_TransformKind != Translation);
...
::AdditionalBlockMatchingSetup(MetricPointer &metric, unsigned int blockNum)
{
    ...
    if ((m_MetricKind == TensorGeneralizedCorrelation)||(m_MetricKind == TensorOrientedGeneralizedCorrelation))
        ((TensorGeneralizedMetricType *)metric.GetPointer())->PreComputeFixedValues();

    if (m_MetricKind == TensorCorrelation)
             ...
             InputImageType *movingImage)
{
    ...
    for (unsigned int block = startBlock; block < endBlock; ++block)
    {
        ...
        if (m_MetricKind != TensorMeanSquares)
        {
            double val = 0;
            switch (m_OptimizerKind)
            {
...
::PrintSelf(std::ostream& os, itk::Indent indent) const
{
    ...
    os << indent << "Minimum Error between transforms: " << m_MinimalTransformError << std::endl;
 ...
 */
template <typename TInputImage>
typename TensorBlockMatchingBaseRegistrationMethod<TInputImage>::TransformOutputType *
TensorBlockMatchingBaseRegistrationMethod<TInputImage>
::GetOutput()
{
