...
double TensorCompartment::GetFourierTransformedDiffusionProfile(
  ...
  double bValue = anima::GetBValueFromAcquisitionParameters(
      smallDelta, bigDelta, gradientStrength);

  return std::exp(-bValue * quadForm);
}

TensorCompartment::ListType &TensorCompartment::GetSignalAttenuationJacobian(
    double smallDelta, double bigDelta, double gradientStrength,
    const Vector3DType &gradient) {
  this->UpdateDiffusionTensor();
  ...
  double signalAttenuation = this->GetFourierTransformedDiffusionProfile(
      smallDelta, bigDelta, gradientStrength, gradient);
  ...
  double DgTe1DTheta =
      m_CosTheta * (gradient[0] * m_CosPhi + gradient[1] * m_SinPhi) -
      gradient[2] * m_SinTheta;
  double DgTe1DPhi =
      m_SinTheta * (gradient[1] * m_CosPhi - gradient[0] * m_SinPhi);

  double DgTe2DTheta = m_SinAlpha * innerProd1;
  double DgTe2DPhi =
      gradient[0] *
          (m_CosTheta * m_SinPhi * m_SinAlpha - m_CosPhi * m_CosAlpha) -
      gradient[1] *
          (m_SinPhi * m_CosAlpha + m_CosTheta * m_CosPhi * m_SinAlpha);
  double DgTe2DAlpha =
      gradient[0] *
          (m_SinPhi * m_SinAlpha - m_CosTheta * m_CosPhi * m_CosAlpha) -
      gradient[1] *
          (m_CosPhi * m_SinAlpha + m_CosTheta * m_SinPhi * m_CosAlpha) +
      gradient[2] * m_SinTheta * m_CosAlpha;

  double diffAxialRadial2 =
      this->GetAxialDiffusivity() - this->GetRadialDiffusivity2();
  double diffRadialDiffusivities =
      this->GetRadialDiffusivity1() - this->GetRadialDiffusivity2();

  // Derivative w.r.t. theta
  double bValue = anima::GetBValueFromAcquisitionParameters(
      smallDelta, bigDelta, gradientStrength);
  m_JacobianVector[0] = -2.0 * bValue *
                        (diffAxialRadial2 * innerProd1 * DgTe1DTheta +
                         diffRadialDiffusivities * innerProd2 * DgTe2DTheta) *
                        signalAttenuation;

  // Derivative w.r.t. phi
  m_JacobianVector[1] = -2.0 * bValue *
                        (diffAxialRadial2 * innerProd1 * DgTe1DPhi +
                         diffRadialDiffusivities * innerProd2 * DgTe2DPhi) *
                        signalAttenuation;

  // Derivative w.r.t. alpha
  m_JacobianVector[2] = -2.0 * bValue * diffRadialDiffusivities * innerProd2 *
                        DgTe2DAlpha * signalAttenuation;
  ...
  if (m_EstimateDiffusivities) {
    ...
    m_JacobianVector[4] = -bValue *
                          (innerProd1 * innerProd1 + innerProd2 * innerProd2) *
                          signalAttenuation;
...
double TensorCompartment::GetLogDiffusionProfile(const Vector3DType &sample) {
  ...
  double resVal =
      -1.5 * std::log(2.0 * M_PI) - 0.5 * std::log(m_TensorDeterminant);
...
TensorCompartment::ListType &TensorCompartment::GetParameterLowerBounds() {
  ...
  std::fill(m_ParametersLowerBoundsVector.begin(),
            m_ParametersLowerBoundsVector.end(), anima::MCMZeroLowerBound);

  if (m_EstimateDiffusivities) {
    m_ParametersLowerBoundsVector[3] =
...
void TensorCompartment::SetCompartmentVector(
    ModelOutputVectorType &compartmentVector) {
  if (compartmentVector.GetSize() != this->GetCompartmentSize())
    itkExceptionMacro(
        "The input vector size does not match the size of the compartment");

  anima::GetTensorFromVectorRepresentation(compartmentVector, m_WorkVnlMatrix1,
                                           m_SpaceDimension);
...
void TensorCompartment::UpdateParametersFromTensor() {
  ...
  itk::SymmetricEigenAnalysis<Matrix3DType, vnl_diag_matrix<double>,
                              vnl_matrix<double>>
      eigSys(m_SpaceDimension);
  ...
  if ((theta != 0) && (theta != M_PI))
    ...
    // Infinite number of solutions for phi from main eigenvector, set it as
    // alpha - pi/2
  ...
  double axialDiff =
      std::max(anima::MCMDiffusivityLowerBound,
               std::min(eVals[2], anima::MCMDiffusivityUpperBound));
  double radialDiff1 =
      std::max(anima::MCMDiffusivityLowerBound,
               std::min(eVals[1], anima::MCMDiffusivityUpperBound));
  double radialDiff2 =
      std::max(anima::MCMDiffusivityLowerBound,
               std::min(eVals[0], anima::MCMDiffusivityUpperBound));
...
unsigned int TensorCompartment::GetCompartmentSize() { return 6; }
...
TensorCompartment::ModelOutputVectorType &
TensorCompartment::GetCompartmentVector() {
  if (m_CompartmentVector.GetSize() != this->GetCompartmentSize())
    m_CompartmentVector.SetSize(this->GetCompartmentSize());

  this->UpdateDiffusionTensor();

  anima::GetVectorRepresentation(m_DiffusionTensor.GetVnlMatrix().as_matrix(),
                                 m_CompartmentVector,
                                 this->GetCompartmentSize());
  return m_CompartmentVector;
}

void TensorCompartment::Reorient(vnl_matrix<double> &orientationMatrix,
  ...
  if (!affineTransform)
    anima::RotateSymmetricMatrix(m_WorkVnlMatrix1, orientationMatrix,
                                 m_WorkVnlMatrix2);
  else {
    // PPD re-orientation
    ...
    itk::SymmetricEigenAnalysis<EigVecMatrixType, EigValVectorType,
                                EigVecMatrixType>
        eigenComputer(3);
    EigVecMatrixType eigVecs;
    EigValVectorType eigVals;

    EigVecMatrixType itkMatrix;
    itkMatrix = m_WorkVnlMatrix1;
    eigenComputer.ComputeEigenValuesAndVectors(itkMatrix, eigVals, eigVecs);
    anima::ExtractPPDRotationFromJacobianMatrix(orientationMatrix,
                                                ppdOrientationMatrix, eigVecs);
    anima::RotateSymmetricMatrix(m_WorkVnlMatrix1, ppdOrientationMatrix,
                                 m_WorkVnlMatrix2);
  }
...
void TensorCompartment::UpdateDiffusionTensor() {
  ...
  for (unsigned int i = 0; i < m_SpaceDimension; ++i)
    for (unsigned int j = i; j < m_SpaceDimension; ++j) {
      m_DiffusionTensor(i, j) +=
          m_EigenVector1[i] * m_EigenVector1[j] * (axialDiff - radialDiff2);
      m_DiffusionTensor(i, j) +=
          m_EigenVector2[i] * m_EigenVector2[j] * (radialDiff1 - radialDiff2);
...
void TensorCompartment::UpdateAngleConfiguration() {
  ...
  m_EigenVector2[0] =
      -1.0 * (m_CosTheta * m_CosPhi * m_SinAlpha + m_SinPhi * m_CosAlpha);
  m_EigenVector2[1] =
      m_CosPhi * m_CosAlpha - m_CosTheta * m_SinPhi * m_SinAlpha;
...
double TensorCompartment::GetApparentFractionalAnisotropy() {
  ...
  double numFA = std::sqrt((l1 - l2) * (l1 - l2) + (l2 - l3) * (l2 - l3) +
                           (l3 - l1) * (l3 - l1));
