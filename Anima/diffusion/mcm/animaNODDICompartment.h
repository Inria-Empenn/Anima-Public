...
private:
  bool m_EstimateOrientationConcentration, m_EstimateAxialDiffusivity,
      m_EstimateExtraAxonalFraction;
  bool m_ChangedConstraints;
  unsigned int m_NumberOfParameters;

  //! Optimization variable: set to true when the internal parameter has been
  //! modified requiring to recompute all quantities depending on it
  bool m_ModifiedParameters;
  bool m_ModifiedConcentration;

  //! Store last used bvalue and gradient to avoid computing expensive values
  //! twice
