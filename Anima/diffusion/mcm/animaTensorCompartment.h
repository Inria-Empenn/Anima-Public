...
private:
  ...
  //! Optimization variable: set to true when an internal parameter is modified
  //! requiring to recompute the tensor
  bool m_ModifiedTensor;

  //! Optimization variable: set to true if diffusion tensor has just been
  //! updated and its inverse needs update
  bool m_UpdateInverseTensor;

  //! Optimization variable: set to true when an internal parameter is modified
  //! requiring to recompute the angles' sine and cosine values
  bool m_ModifiedAngles;

  //! Optimization variable: set to true when compartment vector is set
  //! externally (requires all parameters recompute)
