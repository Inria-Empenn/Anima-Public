...
class DTIEstimationImageFilter
    : public anima::MaskedImageToImageFilter<
          itk::Image<InputPixelScalarType, 3>,
          itk::VectorImage<OutputPixelScalarType, 3>> {
public:
...
protected:
  DTIEstimationImageFilter() : Superclass() {
    m_BValuesList.clear();
  ...
  static double OptimizationFunction(const std::vector<double> &x,
                                     std::vector<double> &grad,
                                     void *func_data);
  double ComputeCostAtPosition(const std::vector<double> &x,
  ...
  double
  ComputeB0AndVarianceFromTensorVector(const vnl_matrix<double> &tensorValue,
