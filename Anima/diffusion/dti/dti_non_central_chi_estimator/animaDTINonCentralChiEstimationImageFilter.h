...
public:
  ...
  using Self = DTINonCentralChiEstimationImageFilter<PixelScalarType>;
...
protected:
  DTINonCentralChiEstimationImageFilter() : Superclass() {
    m_BValuesList.clear();
  ...
  double ComputeLocalSigma(unsigned int nbCoils, double oldLocalSigma,
                           double b0Value, std::vector<double> &dtiValue,
...
private:
  ...
  //! Controls the number of coils, setting it to 1 goes back to Rician noise
  //! assumption
