...
BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::BaseProbabilisticTractographyImageFilter() {
...
template <class TInputModelImageType>
void BaseProbabilisticTractographyImageFilter<TInputModelImageType>::Update() {
  ...
  unsigned int numSteps =
      std::floor(m_PointsToProcess.size() / (double)stepData);
  ...
  for (unsigned int j = 0; j < this->GetNumberOfWorkUnits(); ++j) {
    resultFibers.insert(resultFibers.end(),
                        tmpStr.resultFibersFromThreads[j].begin(),
                        tmpStr.resultFibersFromThreads[j].end());
    resultWeights.insert(resultWeights.end(),
                         tmpStr.resultWeightsFromThreads[j].begin(),
                         tmpStr.resultWeightsFromThreads[j].end());
  }

  std::cout << "\nKept " << resultFibers.size() << " fibers after filtering"
            << std::endl;
  this->createVTKOutput(resultFibers, resultWeights);
}

template <class TInputModelImageType>
void BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::PrepareTractography() {
  if (!m_B0Image)
  ...
  if ((m_InitialColinearityDirection == Outward) ||
      (m_InitialColinearityDirection == Center)) {
    itk::ImageMomentsCalculator<ScalarImageType>::Pointer momentsCalculator =
        itk::ImageMomentsCalculator<ScalarImageType>::New();
  ...
  MaskImageIteratorType maskItr(m_SeedMask,
                                m_InputModelImage->GetLargestPossibleRegion());
  m_PointsToProcess.clear();
  ...
  if (m_FilterMask) {
    MaskImageIteratorType filterItr(
        m_FilterMask, m_InputModelImage->GetLargestPossibleRegion());
    ...
    if (is2d) {
      ...
      for (unsigned int j = 0; j < m_NumberOfFibersPerPixel; ++j) {
        ...
        for (unsigned int i = 0; i < m_NumberOfFibersPerPixel; ++i) {
          ...
          m_SeedMask->TransformContinuousIndexToPhysicalPoint(realIndex,
                                                              tmpPoint);
          tmpFiber[0] = tmpPoint;
  ...
  std::cout << "Generated " << m_PointsToProcess.size()
            << " seed points from ROI mask" << std::endl;
}

template <class TInputModelImageType>
typename BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::InterpolatorType *
BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::GetModelInterpolator() {
  typedef itk::LinearInterpolateImageFunction<InputModelImageType>
      InternalInterpolatorType;

  typename InternalInterpolatorType::Pointer outInterpolator =
      InternalInterpolatorType::New();
...
BaseProbabilisticTractographyImageFilter<TInputModelImageType>::ThreadTracker(
    void *arg) {
  ...
  tmpArg->trackerPtr->ThreadTrack(nbThread,
                                  tmpArg->resultFibersFromThreads[nbThread],
                                  tmpArg->resultWeightsFromThreads[nbThread]);
...
template <class TInputModelImageType>
void BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::ThreadTrack(unsigned int numThread,
                                       FiberProcessVectorType &resultFibers,
                                       ListType &resultWeights) {
  ...
  while (continueLoop) {
    ...
    this->ThreadedTrackComputer(numThread, resultFibers, resultWeights,
                                startPoint, endPoint);
...
template <class TInputModelImageType>
void BaseProbabilisticTractographyImageFilter<TInputModelImageType>::
    ThreadedTrackComputer(unsigned int numThread,
                          FiberProcessVectorType &resultFibers,
  ...
  for (unsigned int i = startSeedIndex; i < endSeedIndex; ++i) {
    bool isInside = m_SeedMask->TransformPhysicalPointToContinuousIndex(
        m_PointsToProcess[i][0], startIndex);

    tmpFibers = this->ComputeFiber(m_PointsToProcess[i], modelInterpolator,
                                   numThread, tmpWeights);
...
template <class TInputModelImageType>
typename BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::FiberProcessVectorType
BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::FilterOutputFibers(FiberProcessVectorType &fibers,
                                              ListType &weights) {
...
template <class TInputModelImageType>
void BaseProbabilisticTractographyImageFilter<TInputModelImageType>::
    createVTKOutput(FiberProcessVectorType &filteredFibers,
  ...
  for (unsigned int i = 0; i < filteredFibers.size(); ++i) {
    ...
    for (unsigned int j = 0; j < npts; ++j) {
      ids[j] = myPoints->InsertNextPoint(filteredFibers[i][j][0],
                                         filteredFibers[i][j][1],
                                         filteredFibers[i][j][2]);
...
typename BaseProbabilisticTractographyImageFilter<
    TInputModelImageType>::FiberProcessVectorType
BaseProbabilisticTractographyImageFilter<TInputModelImageType>::ComputeFiber(
    FiberType &fiber, InterpolatorPointer &modelInterpolator,
  ...
  fiberComputationData.particleWeights =
      ListType(m_NumberOfParticles, 1.0 / m_NumberOfParticles);
  fiberComputationData.stoppedParticles =
      std::vector<bool>(m_NumberOfParticles, false);
  fiberComputationData.classSizes =
      MembershipType(numberOfClasses, m_NumberOfParticles);
  fiberComputationData.classWeights =
      ListType(numberOfClasses, 1.0 / numberOfClasses);
  // We need membership vectors in each direction
  fiberComputationData.classMemberships =
  ...
  while (!stopLoop) {
    ...
    for (unsigned int i = 0; i < m_NumberOfParticles; ++i) {
      ...
      double estimatedB0Value =
          m_B0Interpolator->EvaluateAtContinuousIndex(currentIndex);
      estimatedNoiseValue =
          m_NoiseInterpolator->EvaluateAtContinuousIndex(currentIndex);

      if (!this->CheckModelProperties(estimatedB0Value, estimatedNoiseValue,
      ...
      if (numIter == 1) {
        ...
        previousDirections[i] = this->InitializeFirstIterationFromModel(
            initDir, modelValue, numThread);
      }

      // Propose a new direction based on the previous one and the diffusion
      // information at current position
      double log_prior = 0, log_proposal = 0;
      newDirection = this->ProposeNewDirection(
          previousDirections[i], modelValue, sampling_direction, log_prior,
      ...
      // Log-weight update must be done at new position (except for prior and
      // proposal)
      isInside = m_SeedMask->TransformPhysicalPointToContinuousIndex(
          currentPoint, newIndex);
      ...
      estimatedNoiseValue =
          m_NoiseInterpolator->EvaluateAtContinuousIndex(newIndex);

      // Update the weight of the particle
      double updateWeightLogVal = this->ComputeLogWeightUpdate(
          estimatedB0Value, estimatedNoiseValue, newDirection, modelValue,
          log_prior, log_proposal, numThread);

      logWeightVals[i] =
          updateWeightLogVal + anima::safe_log(oldFiberWeights[i]);
    }
    ...
    for (unsigned int i = 0; i < numberOfClasses; ++i) {
      ...
      unsigned int classSize =
          fiberComputationData.reverseClassMemberships[i].size();
      tmpVector.clear();

      for (unsigned int j = 0; j < classSize; ++j) {
        if (!fiberComputationData.stoppedParticles
                 [fiberComputationData.reverseClassMemberships[i][j]])
          tmpVector.push_back(
              logWeightVals[fiberComputationData
                                .reverseClassMemberships[i][j]]);
      }
    ...
    for (unsigned int i = 0; i < numberOfClasses; ++i) {
      if (!emptyClasses[i])
        tmpVector.push_back(
            anima::safe_log(fiberComputationData.classWeights[i]) +
            logWeightSums[i]);
    }
    ...
    for (unsigned int i = 0; i < numberOfClasses; ++i) {
      if (!emptyClasses[i]) {
        double t =
            std::exp(anima::safe_log(fiberComputationData.classWeights[i]) +
                     logWeightSums[i] - logSumTmpVector);
    ...
    for (unsigned int m = 0; m < numberOfClasses; ++m) {
      if (effectiveNumberOfParticles[m] != 0)
        ...
        continue; // Q: shouldn't we be treating this case as an empty cluster
                  // that shouldn't even exist? (same as previous Q)

      // Actual class resampling
      if (effectiveNumberOfParticles[m] <
          m_ResamplingThreshold * fiberComputationData.classSizes[m]) {
        ...
        for (unsigned int i = 0; i < fiberComputationData.classSizes[m]; ++i) {
          unsigned int posIndex =
              fiberComputationData.reverseClassMemberships[m][i];
          weightSpecificClassValues[i] =
              fiberComputationData.particleWeights[posIndex];
          previousDirectionsCopy[i] = previousDirections[posIndex];
          fiberParticlesCopy[i] = fiberComputationData.fiberParticles[posIndex];
        }

        std::discrete_distribution<> dist(weightSpecificClassValues.begin(),
                                          weightSpecificClassValues.end());
        ...
        for (unsigned int i = 0; i < fiberComputationData.classSizes[m]; ++i) {
          ...
          // In all of this, we suppose that stopped particles have zero weights
          // and will therefore be lost when resampling
        ...
        for (unsigned int i = 0; i < fiberComputationData.classSizes[m]; ++i) {
          ...
          if (!usedFibers[i]) {
            if (oldFiberWeights[iReal] >
                m_FiberTrashThreshold / m_NumberOfParticles) {
              if (fiberComputationData.particleWeights[iReal] != 0)
                fiberParticlesCopy[i].pop_back();

              // The fiber trash used to contain fibers that were lost with a
              // sufficient weight However, using way too much memory so removed
              // for now
              // if (fiberParticlesCopy[i].size() > m_MinLengthFiber /
              // m_StepProgression)
        ...
        // Update only weightVals, oldWeightVals will get updated when starting
        // back the loop Same here for stopped fibers, they get rejected when
        // resampling
        for (unsigned int i = 0; i < fiberComputationData.classSizes[m]; ++i)
          fiberComputationData.particleWeights
              [fiberComputationData.reverseClassMemberships[m][i]] =
              1.0 / fiberComputationData.classSizes[m];
      }
    }

    // We need stopping criterions
    // Length is easy, given that each step is constant we just need to check
    // the fiber size: numIter Example :
    if (numIter > m_MaxLengthFiber / m_StepProgression)
      stopLoop = true;

    numberOfClasses = this->UpdateClassesMemberships(
        fiberComputationData, previousDirections, m_Generators[numThread]);

    for (unsigned int i = 0; i < fiberComputationData.particleWeights.size();
         ++i) {
  ...
  // Now that we're done, if we don't keep individual particles, merge them
  // cluster by cluster
  if (m_MAPMergeFibers) {
    FiberProcessVectorType mergedOutput, classMergedOutput;
    for (unsigned int i = 0; i < numberOfClasses; ++i) {
      this->MergeParticleClassFibers(fiberComputationData, classMergedOutput,
                                     i);
      mergedOutput.insert(mergedOutput.end(), classMergedOutput.begin(),
                          classMergedOutput.end());
    }
...
template <class TInputModelImageType>
unsigned int BaseProbabilisticTractographyImageFilter<TInputModelImageType>::
    UpdateClassesMemberships(FiberWorkType &fiberData,
  ...
  if (numClasses > 1) {
    for (unsigned int i = 0; i < numClasses; ++i) {
      ...
      bool activeClass =
          this->MergeParticleClassFibers(fiberData, mapMergedFibersRef, i);
      if (!activeClass)
        continue;
      ...
      for (unsigned int j = i + 1; j < numClasses; ++j) {
        ...
        double maxVal = 0;
        bool activeSubClass =
            this->MergeParticleClassFibers(fiberData, mapMergedFibersFlo, j);
        if (!activeSubClass)
          continue;
        ...
        case 0: {
          ...
          unsigned int minSizeFiber = std::min(mapMergedFibersRef[0].size(),
                                               mapMergedFibersFlo[0].size());

          for (unsigned int l = 0; l < minSizeFiber; ++l) {
            double positionDist = anima::ComputeEuclideanDistance(
                mapMergedFibersRef[0][l], mapMergedFibersFlo[0][l]);
        ...
        case 1: {
          ...
          for (unsigned int l = 0; l < mapMergedFibersRef[0].size(); ++l) {
            double tmpVal = anima::ComputePointToSetDistance(
                mapMergedFibersRef[0][l], mapMergedFibersFlo[0]);
          ...
          if (maxVal <= m_PositionDistanceFuseThreshold) {
            for (unsigned int l = 0; l < mapMergedFibersFlo[0].size(); ++l) {
              double tmpVal = anima::ComputePointToSetDistance(
                  mapMergedFibersFlo[0][l], mapMergedFibersRef[0]);
        ...
        case 2: {
          ...
          maxVal = anima::ComputeModifiedDirectedHausdorffDistance(
              mapMergedFibersRef[0], mapMergedFibersFlo[0]);

          if (maxVal <= m_PositionDistanceFuseThreshold)
            maxVal = std::max(
                maxVal, anima::ComputeModifiedDirectedHausdorffDistance(
                            mapMergedFibersFlo[0], mapMergedFibersRef[0]));
        ...
        // To do so, an index table (classesFusion) is updated, each of its
        // cells tells to which new class the current class belongs.
        // newNumClasses is the new number of classes
        if (maxVal <= m_PositionDistanceFuseThreshold) {
          newNumClasses--;
    ...
    // mapFusion will hold the correspondance between non contiguous and
    // contiguous indexes
    int maxVal = -1;
  ...
  // Therefore, deciding on cluster splits. The trick here is that we don't want
  // to actually really perform fuse if it is to split right after (for speed
  // reasons). So we'll play with classesFusion indexes all along to keep track
  ...
  std::fill(afterMergeClassesDirections.begin(),
            afterMergeClassesDirections.end(), zeroDirection);

  // Compute average directions after fusion, directions contains the last
  // directions taken by particles
  for (unsigned int i = 0; i < m_NumberOfParticles; ++i) {
  ...
  // From those averaged directions, we compute a dispersion kappa value, that
  // will be used to decide on split
  for (unsigned int i = 0; i < newNumClasses; ++i) {
    double norm = 0;
    for (unsigned int j = 0; j < p; ++j)
      norm +=
          afterMergeClassesDirections[i][j] * afterMergeClassesDirections[i][j];
    norm = sqrt(norm);
    ...
    if (afterMergeNumPoints[i] != 0) {
      ...
      kappa = std::exp(anima::safe_log(R) + anima::safe_log(p - R * R) -
                       anima::safe_log(1 - R * R));
    }

    afterMergeKappaValues.push_back(std::make_pair(i, kappa));
    // We do not allow any split resulting in less than
    // m_MinimalNumberOfParticlesPerClass so testing with respect to
    // 2*m_MinimalNumberOfParticlesPerClass If it is ok, and kappa is small
    // enough, there is too much dispersion inside the cluster -> splitting
    if ((kappa <= m_KappaSplitThreshold) &&
        (afterMergeNumPoints[i] >= 2 * m_MinimalNumberOfParticlesPerClass) &&
        (afterMergeNumPoints[i] != 0))
      numSplits++;
    else
      afterMergeKappaValues[i].second = m_KappaSplitThreshold + 1;
  }

  std::partial_sort(afterMergeKappaValues.begin(),
                    afterMergeKappaValues.begin() + numSplits,
                    afterMergeKappaValues.end(), pair_comparator());
  ...
  for (unsigned int i = 0; i < newNumClasses; ++i) {
    if (!splitClasses[i]) {
      // ith class is just a potential merge of classes. Easy case: just take
      // all particles from classes marked as new ith class
      for (unsigned int j = 0; j < fusedClassesIndexes[i].size(); ++j) {
        unsigned int classIndex = fusedClassesIndexes[i][j];
        for (unsigned int k = 0;
             k < fiberData.reverseClassMemberships[classIndex].size(); ++k) {
          unsigned int particleNumber =
              fiberData.reverseClassMemberships[classIndex][k];
      ...
      if (fusedClassesIndexes[i].size() != 1) {
        ...
        for (unsigned int j = 0; j < fusedClassesIndexes[i].size(); ++j) {
          ...
          for (unsigned int k = 0;
               k < fiberData.reverseClassMemberships[classIndex].size(); ++k)
            newClassWeights[currentIndex] +=
                fiberData.classWeights[classIndex] *
                fiberData.particleWeights
                    [fiberData.reverseClassMemberships[classIndex][k]];
        }
        ...
        for (unsigned int j = 0; j < fusedClassesIndexes[i].size(); ++j) {
          ...
          for (unsigned int k = 0;
            ...
            unsigned int posIndex =
                fiberData.reverseClassMemberships[classIndex][k];
            newParticleWeights[posIndex] =
                std::exp(anima::safe_log(fiberData.classWeights[classIndex]) +
                         anima::safe_log(fiberData.particleWeights[posIndex]) -
                         anima::safe_log(newClassWeights[currentIndex]));
          }
        }
      } else
        newClassWeights[currentIndex] =
            fiberData.classWeights[fusedClassesIndexes[i][0]];

      ++currentIndex;
    } else {
      // ith class is a split at the end. In that case, first gather all
      // particles from classes merged before. Then, plug k-means
      ...
      for (unsigned int j = 0; j < fusedClassesIndexes[i].size(); ++j) {
        ...
        for (unsigned int k = 0;
             k < fiberData.reverseClassMemberships[classIndex].size(); ++k)
          vectorToCluster.push_back(
              fiberData
                  .fiberParticles[fiberData
                                      .reverseClassMemberships[classIndex][k]]
                  .back());
      }
      ...
      while (loopOnClustering) {
        ...
        if ((kmFilter.GetNumberPerClass(0) > 0) &&
            (kmFilter.GetNumberPerClass(1) > 0))
      ...
      for (unsigned int j = 0; j < fusedClassesIndexes[i].size(); ++j) {
        ...
        for (unsigned int k = 0;
          ...
          newClassesMemberships[fiberData
                                    .reverseClassMemberships[classIndex][k]] =
              classPos;
          newReverseClassesMemberships[classPos].push_back(
              fiberData.reverseClassMemberships[classIndex][k]);

          newClassWeights[classPos] +=
              fiberData.classWeights[classIndex] *
              fiberData.particleWeights
                  [fiberData.reverseClassMemberships[classIndex][k]];
      ...
      for (unsigned int j = 0; j < fusedClassesIndexes[i].size(); ++j) {
        ...
        for (unsigned int k = 0;
          ...
          newParticleWeights[posIndex] =
              std::exp(anima::safe_log(fiberData.classWeights[classIndex]) +
                       anima::safe_log(fiberData.particleWeights[posIndex]) -
                       anima::safe_log(newClassWeights[classPos]));
...
template <class TInputModelImageType>
bool BaseProbabilisticTractographyImageFilter<TInputModelImageType>::
    MergeParticleClassFibers(FiberWorkType &fiberData,
  ...
  for (unsigned int j = 0; j < fiberData.classSizes[classNumber]; ++j) {
    if (fiberData.stoppedParticles
            [fiberData.reverseClassMemberships[classNumber][j]])
      stoppedIndexes.push_back(
          fiberData.reverseClassMemberships[classNumber][j]);
    else
      runningIndexes.push_back(
          fiberData.reverseClassMemberships[classNumber][j]);
  }
  ...
  for (unsigned int i = 0; i < stoppedIndexes.size(); ++i) {
    unsigned int particleSize =
        fiberData.fiberParticles[stoppedIndexes[i]].size();
