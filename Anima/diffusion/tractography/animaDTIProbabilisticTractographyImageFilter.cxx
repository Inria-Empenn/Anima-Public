...
DTIProbabilisticTractographyImageFilter::ProposeNewDirection(
    Vector3DType &oldDirection, VectorType &modelValue,
  ...
  bool is2d =
      this->GetInputModelImage()->GetLargestPossibleRegion().GetSize()[2] <= 1;
  ...
  if (LC > m_ThresholdForProlateTensor) {
    ...
    m_WatsonDistribution.SetConcentrationParameter(
        this->GetKappaOfPriorDistribution());
...
DTIProbabilisticTractographyImageFilter::Vector3DType
DTIProbabilisticTractographyImageFilter::InitializeFirstIterationFromModel(
    Vector3DType &colinearDir, VectorType &modelValue, unsigned int threadId) {
  Vector3DType resVec, tmpVec;
  bool is2d =
      (this->GetInputModelImage()->GetLargestPossibleRegion().GetSize()[2] ==
       1);

  itk::SymmetricEigenAnalysis<Matrix3DType, Vector3DType, Matrix3DType>
      EigenAnalysis(InputModelImageType::ImageDimension);
...
bool DTIProbabilisticTractographyImageFilter::CheckModelProperties(
    double estimatedB0Value, double estimatedNoiseValue, VectorType &modelValue,
...
double DTIProbabilisticTractographyImageFilter::ComputeLogWeightUpdate(
    double b0Value, double noiseValue, Vector3DType &newDirection,
    VectorType &modelValue, double &log_prior, double &log_proposal,
    unsigned int threadId) {
  bool is2d =
      this->GetInputModelImage()->GetLargestPossibleRegion().GetSize()[2] <= 1;
...
void DTIProbabilisticTractographyImageFilter::SetKappaPolynomialCoefficients(
...
void DTIProbabilisticTractographyImageFilter::GetDTIPrincipalDirection(
    const VectorType &modelValue, Vector3DType &resVec, bool is2d) {
  itk::SymmetricEigenAnalysis<Matrix3DType, Vector3DType, Matrix3DType>
      EigenAnalysis(InputModelImageType::ImageDimension);
  EigenAnalysis.SetOrderEigenValues(true);
...
void DTIProbabilisticTractographyImageFilter::GetDTIMinorDirection(
    VectorType &modelValue, Vector3DType &resVec) {
  itk::SymmetricEigenAnalysis<Matrix3DType, Vector3DType, Matrix3DType>
      EigenAnalysis(InputModelImageType::ImageDimension);
  EigenAnalysis.SetOrderEigenValues(true);
...
void DTIProbabilisticTractographyImageFilter::ComputeModelValue(
    InterpolatorPointer &modelInterpolator, ContinuousIndexType &index,
...
double DTIProbabilisticTractographyImageFilter::GetLinearCoefficient(
    VectorType &modelValue) {
  itk::SymmetricEigenAnalysis<Matrix3DType, Vector3DType, Matrix3DType>
      EigenAnalysis(InputModelImageType::ImageDimension);
  EigenAnalysis.SetOrderEigenValues(true);
...
double DTIProbabilisticTractographyImageFilter::GetFractionalAnisotropy(
    VectorType &modelValue) {
  itk::SymmetricEigenAnalysis<Matrix3DType, Vector3DType, Matrix3DType>
      EigenAnalysis(InputModelImageType::ImageDimension);
  EigenAnalysis.SetOrderEigenValues(true);
...
void DTIProbabilisticTractographyImageFilter::GetEigenValueCombinations(
    VectorType &modelValue, double &meanLambda, double &perpLambda) {
  itk::SymmetricEigenAnalysis<Matrix3DType, Vector3DType, Matrix3DType>
      EigenAnalysis(InputModelImageType::ImageDimension);
  EigenAnalysis.SetOrderEigenValues(true);
...
void DTIProbabilisticTractographyImageFilter::ComputeAdditionalScalarMaps() {
  ...
  itk::SymmetricEigenAnalysis<MatrixType, vnl_diag_matrix<double>, MatrixType>
      EigenAnalysis(3);
  ...
  for (unsigned int i = 0; i < numPoints; ++i) {
    ...
    bool isInside =
        this->GetInputModelImage()->TransformPhysicalPointToContinuousIndex(
            tmpPoint, tmpIndex);
