...
bool dtiTractographyImageFilter::CheckModelCompatibility(
    VectorType &modelValue, itk::ThreadIdType threadId) {
...
bool dtiTractographyImageFilter::CheckIndexInImageBounds(
    ContinuousIndexType &index) {
  return m_DTIInterpolator->IsInsideBuffer(index);
}

void dtiTractographyImageFilter::GetModelValue(ContinuousIndexType &index,
...
dtiTractographyImageFilter::GetModelPrincipalDirections(
    VectorType &modelValue, bool is2d, itk::ThreadIdType threadId) {
  typedef vnl_matrix<double> MatrixType;
...
dtiTractographyImageFilter::GetNextDirection(PointType &previousDirection,
                                             VectorType &modelValue, bool is2d,
                                             itk::ThreadIdType threadId) {
  PointType newDirection =
      this->GetModelPrincipalDirections(modelValue, is2d, threadId)[0];
  ...
  for (unsigned int i = 0; i < 3; ++i)
    newDirection[i] =
        newDirection[i] * linearCoefficient +
        (1.0 - linearCoefficient) *
            ((1.0 - this->GetPunctureWeight()) * previousDirection[i] +
             this->GetPunctureWeight() * advectedDirection[i]);
...
void dtiTractographyImageFilter::ComputeAdditionalScalarMaps() {
  ...
  anima::LogEuclideanTensorCalculator<double>::Pointer leCalculator =
      anima::LogEuclideanTensorCalculator<double>::New();
