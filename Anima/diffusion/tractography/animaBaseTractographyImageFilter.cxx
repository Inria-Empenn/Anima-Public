...
void BaseTractographyImageFilter::Update() {
  ...
  std::cout << "\nTracked a total of " << resultFibers.size() << " fibers"
            << std::endl;
  resultFibers = this->FilterOutputFibers(resultFibers);
  std::cout << "Kept " << resultFibers.size() << " fibers after filtering"
            << std::endl;
...
void BaseTractographyImageFilter::PrepareTractography() {
  ...
  MaskImageIteratorType seedItr(m_SeedingImage,
  ...
  if (m_FilteringImage.IsNotNull()) {
    MaskImageIteratorType filterItr(m_FilteringImage,
                                    m_InputImage->GetLargestPossibleRegion());
...
BaseTractographyImageFilter::ThreadTracker(void *arg) {
...
void BaseTractographyImageFilter::ThreadTrack(
  ...
  while (continueLoop) {
    ...
    double ratio = std::floor(m_NumberOfProcessedPoints * 100.0 /
                              highestToleratedSeedIndex) /
                   100.0;
...
void BaseTractographyImageFilter::ThreadedTrackComputer(
    unsigned int numThread, std::vector<FiberType> &resultFibers,
  ...
  for (unsigned int i = startSeedIndex; i < endSeedIndex; ++i) {
    ...
    m_SeedingImage->TransformContinuousIndexToPhysicalPoint(
        m_PointsToProcess[i], initialPoint);
    bool isInside = m_SeedingImage->TransformPhysicalPointToIndex(
        initialPoint, curNearestIndex);
    ...
    initialDirections =
        this->GetModelPrincipalDirections(modelValue, is2d, numThread);
...
BaseTractographyImageFilter::FilterOutputFibers(
    std::vector<FiberType> &fibers) {
...
void BaseTractographyImageFilter::createVTKOutput(
    std::vector<FiberType> &filteredFibers) {
  m_Output = vtkPolyData::New();
  m_Output->Initialize();
  m_Output->Allocate();
...
void BaseTractographyImageFilter::ComputeFiber(
    BaseTractographyImageFilter::FiberType &fiber, PointType &initialDirection,
  ...
  while (continueLoop) {
    ...
    this->ComputeNewFiberPoint(fiber[fiber.size() - 1], newDir, curPoint,
                               threadId);

    bool isInside = m_SeedingImage->TransformPhysicalPointToContinuousIndex(
        curPoint, curIndex);
    isInside = m_SeedingImage->TransformPhysicalPointToIndex(curPoint,
                                                             curNearestIndex);
    ...
    if (!m_ForbiddenMaskImage.IsNull()) {
      if (!this->CheckIndexInImageBounds(curNearestIndex,
                                         m_ForbiddenMaskImage)) {
        fiber.clear();
        return;
      }
  ...
  while (continueLoop) {
    ...
    isInside = m_SeedingImage->TransformPhysicalPointToContinuousIndex(
        curPoint, curIndex);
    isInside = m_SeedingImage->TransformPhysicalPointToIndex(curPoint,
                                                             curNearestIndex);
...
void BaseTractographyImageFilter::ComputeNewFiberPoint(
    PointType &oldPoint, PointType &newDirection, PointType &newPoint,
    itk::ThreadIdType threadId) {
  ...
  isInside = m_SeedingImage->TransformPhysicalPointToContinuousIndex(kPosition,
                                                                     curIndex);
  if (!this->CheckIndexInImageBounds(curIndex)) {
    for (unsigned int i = 0; i < 3; ++i)
      newPoint[i] =
          oldPoint[i] + m_StepProgression * (k1[i] + 2.0 * k2[i]) / 3.0;
  ...
  if (!this->CheckIndexInImageBounds(curIndex)) {
    for (unsigned int i = 0; i < 3; ++i)
      newPoint[i] = oldPoint[i] + m_StepProgression *
                                      (k1[i] + 2.0 * k2[i] + 2.0 * k3[i]) / 5.0;
  ...
  for (unsigned int i = 0; i < 3; ++i)
    newPoint[i] = oldPoint[i] + (k1[i] + 2.0 * k2[i] + 2.0 * k3[i] + k4[i]) *
                                    m_StepProgression / 6.0;
}

bool BaseTractographyImageFilter::CheckIndexInImageBounds(
    IndexType &index, ImageBaseType *testImage) {
