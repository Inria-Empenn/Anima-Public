...
void FilterTracks(
    vtkPolyData *tracks, unsigned int startIndex, unsigned int endIndex,
    itk::NearestNeighborInterpolateImageFunction<itk::Image<unsigned short, 3>>
        *interpolator,
  ...
  for (unsigned int i = startIndex; i < endIndex; ++i) {
    ...
    unsigned int upIndexStart =
        std::max(5, static_cast<int>(std::floor(numCellPts / 20.0)));
    ...
    for (unsigned int j = 0; j < upIndexStart; ++j) {
      ...
      bool isInside = interpolator->GetInputImage()
                          ->TransformPhysicalPointToContinuousIndex(
                              pointPosition, currentIndex);
      if (interpolator->IsInsideBuffer(currentIndex)) {
        unsigned int value = static_cast<unsigned int>(
            std::round(interpolator->EvaluateAtContinuousIndex(currentIndex)));
...
ITK_THREAD_RETURN_FUNCTION_CALL_CONVENTION ThreadFilterer(void *arg) {
  ...
  FilterTracks(tmpArg->tracks, startIndex, endIndex, tmpArg->interpolator,
               tmpArg->touchLabels, tmpArg->endingsLabels,
               tmpArg->forbiddenLabels);
...
int main(int argc, char **argv) {
  TCLAP::CmdLine cmd(
      "Filters fibers from a vtp file using a label image and specifying with "
      "several -t and -f which labels should be touched or are forbidden for "
      "each fiber. INRIA / IRISA - VisAGeS/Empenn Team",
      ' ', ANIMA_VERSION);

  TCLAP::ValueArg<std::string> inArg("i", "input", "input tracks file", true,
                                     "", "input tracks", cmd);
  TCLAP::ValueArg<std::string> roiArg("r", "roi", "input ROI label image", true,
                                      "", "ROI image", cmd);
  TCLAP::ValueArg<std::string> outArg("o", "output", "output tracks name", true,
                                      "", "output tracks", cmd);

  TCLAP::MultiArg<unsigned int> touchArg("t", "touch",
                                         "Labels that have to be touched",
                                         false, "touched labels", cmd);
  TCLAP::MultiArg<unsigned int> endingsArg(
      "e", "endings",
      "Labels that have to be touched by the endings of the fibers", false,
      "endings labels", cmd);
  TCLAP::MultiArg<unsigned int> forbiddenArg(
      "f", "forbid", "Labels that must not to be touched", false,
  ...
  ROIImageType::Pointer roiImage =
      anima::readImage<ROIImageType>(roiArg.getValue());

  typedef itk::NearestNeighborInterpolateImageFunction<ROIImageType>
      InterpolatorType;
  ...
  if (endingsLabels.size() > 2)
    std::cerr << "Endings consider only the two ending points of each fiber. "
                 "Having more than two labels will lead to empty bundles"
              << std::endl;
  ...
  vtkSmartPointer<vtkCleanPolyData> vtkCleaner =
      vtkSmartPointer<vtkCleanPolyData>::New();
  vtkCleaner->SetInputData(tracks);
  vtkCleaner->Update();
  tracks->ShallowCopy(vtkCleaner->GetOutput());

  std::cout << "Kept " << tracks->GetNumberOfCells() << " after filtering"
            << std::endl;
