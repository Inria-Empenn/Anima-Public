                  ...
                  const std::vector <unsigned int> &touchLabels, const std::vector <unsigned int> &forbiddenLabels)
{
    ...
    for (unsigned int i = startIndex;i < endIndex;++i)//loop on the tracks
    {
        // Inspect i-th cell
        ...
        seenLabels.clear();
        bool lineOk = true;
		for (unsigned int j = 0;j < numCellPts;++j)//loop on the points belonging to jth track
        {
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Kept " << tracks->GetNumberOfCells() << " after filtering" << std::endl;/*J*/
	std::ofstream fichier1("tract_number.txt");  /*J*/
 
    if(fichier1)/*J*/
        {
            fichier1 << tracks->GetNumberOfCells() << endl;
            fichier1.close();
        }
        else
	...
	std::ofstream fichier2("tract_length.txt"); /*J*/
	int numberPoints[numCells];
	
	for (unsigned int i = 0;i < numCells;++i)//loop on the points belonging to jth track
        {
			vtkCell *cell = tracks->GetCell(i);
			vtkPoints *cellPts = cell->GetPoints();
			vtkIdType numCellPts = cellPts->GetNumberOfPoints();
			numberPoints[i]=numCellPts;fichier2 << numCellPts<< endl;
		}/*J*/
