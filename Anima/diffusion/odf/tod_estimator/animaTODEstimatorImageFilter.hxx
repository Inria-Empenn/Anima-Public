...
void TODEstimatorImageFilter<ScalarType>::BeforeThreadedGenerateData() {
  Superclass::BeforeThreadedGenerateData();

  ReferenceImagePointerType refImg =
      anima::readImage<ReferenceImageType>(m_ReferenceFileName);
  m_LOrder = 8;
  ...
  for (iter->GoToFirstCell(); !iter->IsDoneWithTraversal();
    ...
    for (int i = 0; i < nbPoints; i++) {
      ...
      bool isInside = output->TransformPhysicalPointToIndex(point, index);
...
template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::ComputeCoefs() {
  ...
  double c1 =
      -sqrt(5) *
      (3 * sqrt(l2) * (d + l2) * (pi + atan((d - l2) / (2 * sqrt(l2 * d)))) -
       4 * sqrt(d) * (2 * d + 3 * l2)) /
      (16 * sqrt(pi) * pow(d, 1.5));
  double c2 =
      3 *
      (4 * sqrt(d) * (16 * pow(d, 2) + 115 * d * l2 + 105 * pow(l2, 2)) -
       15 * sqrt(l2) * (3 * pow(d, 2) + 10 * d * l2 + 7 * pow(l2, 2)) *
           (pi + 2 * atan((d - l2) / (2 * sqrt(d * l2))))) /
      (128 * spi * pow(d, 2.5));
  double c3 = -105 * sqrt(13) *
              (at * sqrt(l2) *
                   (5 * pow(d, 3) + 35 * l2 * pow(d, 2) + 63 * pow(l2, 2) * d +
                    33 * pow(l2, 3)) -
               4 * sqrt(d) *
                   (33 * pow(l2, 3) + 52 * d * pow(l2, 2) +
                    103 * pow(d, 2) * l2 / 5 + 128 * pow(d, 3) / 105)) /
              (1024 * spi * pow(d, 3.5));
  double c4 = -315 * sqrt(17) *
              (at * sqrt(l2) *
                   (35 * pow(d, 4) + 420 * pow(d, 3) * l2 +
                    1386 * pow(d, 2) * pow(l2, 2) + 1716 * pow(l2, 3) * d +
                    715 * pow(l2, 4)) -
               4 * sqrt(d) *
                   (715 * pow(l2, 4) + 4433 * pow(l2, 3) * d / 3 +
                    957 * pow(l2, 2) * pow(d, 2) + 6967 * l2 * pow(d, 3) / 35 +
                    2048 * pow(d, 4) / 315)) /
              (16384 * spi * pow(d, 4.5));
  double c5 =
      -24255 * sqrt(21) *
      (at * sqrt(l2) *
           (9 * pow(d, 5) + 165 * pow(d, 4) * l2 +
            858 * pow(d, 3) * pow(l2, 2) + 12870 * pow(d, 2) * pow(l2, 3) / 7 +
            12155 * d * pow(l2, 4) / 7 + 4199 * pow(l2, 5) / 7) -
       4 * sqrt(d) *
           (4199 * pow(l2, 5) / 7 + 32266 * pow(l2, 4) * d / 21 +
            20691 * pow(l2, 3) * pow(d, 2) / 15 +
            24830 * pow(l2, 2) * pow(d, 3) / 49 + 28799 * l2 * pow(d, 4) / 441 +
            32768 * pow(d, 5) / 24255)) /
      (262144 * pow(d, 5.5) * spi);
...
template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::DynamicThreadedGenerateData(
    const TODEstimatorImageFilter::OutputImageRegionType
        &outputRegionForThread) {
  OutputImageType *output = this->GetOutput();
  itk::ImageRegionIterator<OutputImageType> outItr(output,
                                                   outputRegionForThread);
  ...
  while (!outItr.IsAtEnd()) {
    ...
    int tmpSize =
        m_ImgDir[index[0] + Xmax * (index[1] + Ymax * index[2])].size();
    if (tmpSize != 0) {
...
template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::GetMainDirections(
    DirVectorType inDirs, DirVectorType &mainDirs) {
  ...
  while (It < numIt && !stop) {
    ...
    for (int i = 0; i < numDirs; i++) {
      ...
      for (int j = 0; j < numClusters; j++) {
        dist = std::sqrt((clustersAverage[j][0] - inDirs[i][0]) *
                             (clustersAverage[j][0] - inDirs[i][0]) +
                         (clustersAverage[j][1] - inDirs[i][1]) *
                             (clustersAverage[j][1] - inDirs[i][1]) +
                         (clustersAverage[j][2] - inDirs[i][2]) *
                             (clustersAverage[j][2] - inDirs[i][2]));
        if (dist < minDist) {
          minDist = dist;
          clusters[i] = j;
...
TODEstimatorImageFilter<ScalarType>::GetNewClusterAverage(
    int numCluster, DirVectorType &indirs, std::vector<int> &cluster) {
  DirType sum, sumNorm;
  DirVectorType dirs;
  dirs = indirs;
  for (int j = 0; j < 3; j++)
...
template <typename ScalarType>
double TODEstimatorImageFilter<ScalarType>::GetEuclideanDistance(DirType dir1,
                                                                 DirType dir2) {
  return std::sqrt(std::pow((dir2[0] - dir1[0]), 2) +
                   std::pow((dir2[1] - dir1[1]), 2) +
                   std::pow((dir2[2] - dir1[2]), 2));
}

template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::GetSHCoef(
    DirType tmpDir, OutputImagePixelType &coefs) {
  ...
  itk::Matrix<double> itkRotationMatrix =
      anima::GetRotationMatrixFromVectors(m_CstDir, dir);
  ...
  for (unsigned int l = 0; l <= m_LOrder; l += 2) {
    anima::EstimateLocalODFRotationMatrix(ODFRotationMatrix, l, eulerAngles[0],
    ...
    for (unsigned int m = 0; m <= 2 * l; ++m) {
      ...
      for (unsigned int mp = 0; mp <= 2 * l; ++mp)
        rotatedModel[mBaseInd + m] +=
            ODFRotationMatrix(m, mp) * tmp[mBaseInd + mp];
    }
  }
  coefs = rotatedModel;
}
...
template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::AverageODFs(
    std::vector<OutputImagePixelType> &vecCoefs, OutputImagePixelType &resOdf) {
...
template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::PrecomputeSH() {
  int k = 0;
  ...
  for (int i = 0; i < m_NbSample; i++) {
    ...
    for (double l = 0; l <= m_LOrder; l += 2) {
      for (double m = -l; m <= l; m++) {
        m_SpherHarm(k, c++) =
            m_ODFSHBasis->getNthSHValueAtPosition(l, m, tmpDir[0], tmpDir[1]);
      }
    }
    k++;
  }
...
template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::DiscretizeODF(
    OutputImagePixelType ODFCoefs, std::vector<double> &ODFDiscret) {
  int k = 0;
  double resVal2 = 0;
...
TODEstimatorImageFilter<ScalarType>::GetSquareRootODF(
    std::vector<double> ODFDiscret) {
  ...
  Coef = vnl_matrix_inverse<double>(m_SpherHarm.transpose() * m_SpherHarm)
             .as_matrix() *
         m_SpherHarm.transpose() * Odf;
...
template <typename ScalarType>
typename TODEstimatorImageFilter<ScalarType>::OutputImagePixelType
TODEstimatorImageFilter<ScalarType>::GetSquareODF(
    std::vector<double> ODFDiscret) {
  vnl_matrix<double> Odf(m_NbSample, 1);
  vnl_matrix<double> Coef(m_VectorLength, 1);
...
template <typename ScalarType>
void TODEstimatorImageFilter<ScalarType>::GetAverageCoefs(
    std::vector<OutputImagePixelType> &vecCoefs,
    OutputImagePixelType &avgCoef) {
  int numImage = vecCoefs.size();

  std::vector<std::vector<double>> arrayCoef(numImage), arrayLogMap(numImage);
  std::vector<double> expMap(m_VectorLength), mean(m_VectorLength),
      nextMean(m_VectorLength), tangent(m_VectorLength);
...
template <typename ScalarType>
typename TODEstimatorImageFilter<ScalarType>::DirType
TODEstimatorImageFilter<ScalarType>::GetFiberDirection(int index,
                                                       FiberType &fiber) {
  DirType resDir;
...
TODEstimatorImageFilter<ScalarType>::GetCenterVoxel(int index,
                                                    FiberType &fiber) {
  PointType resPoint;
  for (int i = 0; i < 3; i++) {
...
TODEstimatorImageFilter<ScalarType>::ReadFiber(vtkIdType numberOfPoints,
                                               const vtkIdType *indices,
                                               vtkPoints *points) {
  FiberType fiber;
