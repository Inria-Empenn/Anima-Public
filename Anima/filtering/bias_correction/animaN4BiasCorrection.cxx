...
int main(int argc, char *argv[])
{
   ...
   TCLAP::ValueArg<std::string> oArgOutputName("o", "outputBase", "Base name for output files", true, "", "string", cmd);

   TCLAP::ValueArg<std::string> oArgIterationsString("I", "iterations", "Table of number of iterations, default=50x40x30", false, "50x40x30", "string", cmd);

   TCLAP::ValueArg<int> oArgShrinkFactors("S", "shrinkFactor", "Shrink factor, default=4", false, 4, "int", cmd);

   TCLAP::ValueArg<float> oArgWienerFilterNoise("W", "wiener", "Wiener Filter Noise, default=0.01", false, 0.01, "float", cmd);

   TCLAP::ValueArg<float> oArgbfFWHM("B", "bfFWHM", "Bias field Full Width at Half Maximum, default=0.15", false, 0.15, "float", cmd);

   TCLAP::ValueArg<float> oArgConvergenceThreshold("T", "threshold", "Convergence Threshold, default=0.0001", false, 0.0001, "float", cmd);

   TCLAP::ValueArg<int> oArgSplineOrder("O", "splineOrder", "BSpline Order, default=3", false, 3, "int", cmd);

   TCLAP::ValueArg<float> oArgSplineDistance("D", "splineDistance", "B-Spline distance, default=0.0", false, 0.0, "float", cmd);

   TCLAP::ValueArg<std::string> oArgInitialMeshResolutionString("G", "splineGrid", "B-Spline grid resolution. It is ignored if splineDistance>0 or if dimention of it <>3, default=1x1x1", false, "1x1x1", "string", cmd);
