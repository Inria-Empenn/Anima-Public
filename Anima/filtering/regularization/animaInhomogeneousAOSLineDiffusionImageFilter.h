...
public:
  ...
  using Self = InhomogeneousAOSLineDiffusionImageFilter;
  ...
  using RealType = typename itk::NumericTraits<InputPixelType>::RealType;
  using ScalarRealType =
      typename itk::NumericTraits<InputPixelType>::ScalarRealType;
  ...
  void SetDiffusionScalarsImage(DiffusionScalarsImageType *data) {
    m_DiffusionScalarsImage = data;
  }
...
protected:
  ...
  /** InhomogeneousAOSLineDiffusionImageFilter needs all of the input only in
   * the "Direction" dimension. Therefore we enlarge the output's
   ...
   * parameters "outs" and "data". l_coefs, m_coefs and r_coefs are temporary
   * variables to hold LR decomposition results. */
  void FilterDataArray(std::vector<RealType> &outs,
                       const std::vector<RealType> &data,
                       std::vector<RealType> &diffs,
                       std::vector<RealType> &scratch,
                       std::vector<RealType> &l_coefs,
                       std::vector<RealType> &m_coefs,
                       std::vector<RealType> &r_coefs);

  template <typename T1, typename T2>
  void InitializeLeftSideCoefs(T1 &outL0, T1 &outM0, T1 &outR0, const T2 &delta,
                               const T1 &diffs0, const T1 &diffs1) {
    outL0 = 0;
  ...
  void InitializeLeftSideCoefs(itk::Vector<T1, TDimension> &outL0,
                               itk::Vector<T1, TDimension> &outM0,
                               itk::Vector<T1, TDimension> &outR0,
                               const T2 &delta,
                               const itk::Vector<T1, TDimension> &diffs0,
                               const itk::Vector<T1, TDimension> &diffs1) {
    for (unsigned int i = 0; i < TDimension; ++i) {
      outL0[i] = 0;
      outM0[i] = 1.0 + 2 * delta * diffs0[i] / m_SquareGradientDelta;
      outR0[i] =
          -delta * (5 * diffs0[i] - diffs1[i]) / (4 * m_SquareGradientDelta);
    }
  }

  template <typename T1, typename T2>
  void InitializeIthCoefs(T1 &outLi, T1 &outMi, T1 &outRi, const T2 &delta,
                          const T1 &inPrevM, const T1 &inPrevR,
                          const T1 &diffsPrev, const T1 &diffs,
                          const T1 &diffsNext) {
    outLi = 1.0 / inPrevM;
    outLi *= -delta * (diffs + (diffsNext - diffsPrev) / 4.0) /
             m_SquareGradientDelta;

    outMi = 1.0 + 2 * delta * diffs / m_SquareGradientDelta;
    outMi -= outLi * inPrevR;

    outRi = -delta * (diffs - (diffsNext - diffsPrev) / 4.0) /
            m_SquareGradientDelta;
  }

  template <typename T1, typename T2, unsigned int TDimension>
  void InitializeIthCoefs(itk::Vector<T1, TDimension> &outLi,
    ...
    for (unsigned int i = 0; i < TDimension; ++i) {
      ...
      outLi[i] *= -delta * (diffs[i] + (diffsNext[i] - diffsPrev[i]) / 4.0) /
                  m_SquareGradientDelta;

      outMi[i] = 1.0 + 2 * delta * diffs[i] / m_SquareGradientDelta;
      outMi[i] -= outLi[i] * inPrevR[i];

      outRi[i] = -delta * (diffs[i] - (diffsNext[i] - diffsPrev[i]) / 4.0) /
                 m_SquareGradientDelta;
    }
  }

  template <typename T1, typename T2>
  void InitializeRightSideCoefs(T1 &outL, T1 &outM, T1 &outR, const T2 &delta,
                                const T1 &inPrevM, const T1 &inPrevR,
                                const T1 &diffsPrev, const T1 &diffs) {
    outL = 1.0 / inPrevM;
  ...
  template <typename T1, typename T2, unsigned int TDimension>
  void InitializeRightSideCoefs(itk::Vector<T1, TDimension> &outL,
    ...
    for (unsigned int i = 0; i < TDimension; ++i) {
      ...
      outL[i] *= -delta * (5 * diffs[i] - diffsPrev[i]) /
                 (4.0 * m_SquareGradientDelta);
  ...
  void ComputeForwardBacwardSubstitution(std::vector<T1> &outs,
                                         std::vector<T1> &scratch,
                                         const std::vector<T1> &data,
                                         const std::vector<T1> &l_coefs,
                                         const std::vector<T1> &m_coefs,
  ...
  void ComputeForwardBacwardSubstitution(
      std::vector<itk::Vector<T1, TDimension>> &outs,
    ...
    for (int i = ln - 2; i >= 0; i--) {
      for (unsigned int j = 0; j < TDimension; ++j)
        outs[i][j] =
            (scratch[i][j] - r_coefs[i][j] * outs[i + 1][j]) / m_coefs[i][j];
    }
  }
