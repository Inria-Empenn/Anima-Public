...
template <typename TInputImage, typename TDiffusionScalarImage,
          typename TOutputImage>
InhomogeneousAOSLineDiffusionImageFilter<
    TInputImage, TDiffusionScalarImage,
    TOutputImage>::InhomogeneousAOSLineDiffusionImageFilter() {
  m_Direction = 0;
...
template <typename TInputImage, typename TDiffusionScalarImage,
          typename TOutputImage>
void InhomogeneousAOSLineDiffusionImageFilter<
    TInputImage, TDiffusionScalarImage,
    TOutputImage>::SetInputImage(const TInputImage *input) {
...
template <typename TInputImage, typename TDiffusionScalarImage,
          typename TOutputImage>
const TInputImage *
InhomogeneousAOSLineDiffusionImageFilter<TInputImage, TDiffusionScalarImage,
                                         TOutputImage>::GetInputImage() {
...
void InhomogeneousAOSLineDiffusionImageFilter<
    TInputImage, TDiffusionScalarImage,
    TOutputImage>::FilterDataArray(std::vector<RealType> &outs,
  ...
  this->InitializeLeftSideCoefs(l_coefs[0], m_coefs[0], r_coefs[0], delta,
                                diffs[0], diffs[1]);
  ...
  for (unsigned int i = 1; i < ln - 1; ++i) {
    this->InitializeIthCoefs(l_coefs[i], m_coefs[i], r_coefs[i], delta,
                             m_coefs[i - 1], r_coefs[i - 1], diffs[i - 1],
                             diffs[i], diffs[i + 1]);
  }

  // Compute last l, m, r coefs
  this->InitializeRightSideCoefs(l_coefs[ln - 1], m_coefs[ln - 1],
                                 r_coefs[ln - 1], delta, m_coefs[ln - 2],
                                 r_coefs[ln - 2], diffs[ln - 2], diffs[ln - 1]);

  // Now compute forward substitution inside scratch
  this->ComputeForwardBacwardSubstitution(outs, scratch, data, l_coefs, m_coefs,
                                          r_coefs);
}
...
void InhomogeneousAOSLineDiffusionImageFilter<
    TInputImage, TDiffusionScalarImage,
    TOutputImage>::EnlargeOutputRequestedRegion(itk::DataObject *output) {
  TOutputImage *out = dynamic_cast<TOutputImage *>(output);

  if (out) {
    OutputImageRegionType outputRegion = out->GetRequestedRegion();
    const OutputImageRegionType &largestOutputRegion =
    ...
    outputRegion.SetIndex(m_Direction,
                          largestOutputRegion.GetIndex(m_Direction));
...
void InhomogeneousAOSLineDiffusionImageFilter<
  ...
  m_SquareGradientDelta = (inputImage->GetSpacing()[m_Direction]) *
                          (inputImage->GetSpacing()[m_Direction]);
}

template <typename TInputImage, typename TDiffusionScalarImage,
          typename TOutputImage>
void InhomogeneousAOSLineDiffusionImageFilter<
    TInputImage, TDiffusionScalarImage, TOutputImage>::GenerateData() {
  ...
  this->GetMultiThreader()
      ->template ParallelizeImageRegionRestrictDirection<
          TOutputImage::ImageDimension>(
          this->m_Direction, region,
...
void InhomogeneousAOSLineDiffusionImageFilter<
    TInputImage, TDiffusionScalarImage,
    TOutputImage>::DynamicThreadedGenerateData(const OutputImageRegionType
                                                   &outputRegionForThread) {
  typedef typename TOutputImage::PixelType OutputPixelType;

  typedef itk::ImageLinearConstIteratorWithIndex<TInputImage>
      InputConstIteratorType;
  typedef itk::ImageLinearConstIteratorWithIndex<TDiffusionScalarImage>
      DiffusionScalarConstIteratorType;
  ...
  DiffusionScalarConstIteratorType diffScalarsIterator(m_DiffusionScalarsImage,
                                                       region);
  ...
  try // this try is intended to catch an eventual AbortException.
  {
    ...
    while (!inputIterator.IsAtEnd()) {
      ...
      this->FilterDataArray(outs, inps, diffs, scratch, l_coefs, m_coefs,
                            r_coefs);
