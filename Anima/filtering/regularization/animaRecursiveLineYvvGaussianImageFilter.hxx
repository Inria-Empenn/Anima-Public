...
RecursiveLineYvvGaussianImageFilter<
    TInputImage, TOutputImage>::RecursiveLineYvvGaussianImageFilter() {
  m_Direction = 0;
 ...
 */
template <typename TInputImage, typename TOutputImage>
void RecursiveLineYvvGaussianImageFilter<
    TInputImage, TOutputImage>::SetInputImage(const TInputImage *input) {
 ...
 */
template <typename TInputImage, typename TOutputImage>
void RecursiveLineYvvGaussianImageFilter<TInputImage, TOutputImage>::SetUp(
    ScalarRealType spacing) {
  ...
  if (sigmad >= 3.556)
    ...
    if (sigmad < 0.5)
      std::cerr
          << "Too low sigma value (< 0.5), computation will not be precise."
          << std::endl;
  ...
  m_B1 = q *
         (2 * m0 * m1 + m1 * m1 + m2 * m2 + (2 * m0 + 4 * m1) * q + 3 * q * q) /
         scale;
  ...
  m_MMatrix(2, 1) = m_B1 * m_B2 + m_B3 * m_B2 * m_B2 - m_B1 * m_B3 * m_B3 -
                    m_B3 * m_B3 * m_B3 - m_B3 * m_B2 + m_B3;
  m_MMatrix(2, 2) = m_B3 * (m_B1 + m_B3 * m_B2);

  m_MMatrix /= (1 + m_B1 - m_B2 + m_B3) * (1 - m_B1 - m_B2 - m_B3) *
               (1 + m_B2 + (m_B1 - m_B3) * m_B3);
}
...
template <typename TInputImage, typename TOutputImage>
void RecursiveLineYvvGaussianImageFilter<
    TInputImage, TOutputImage>::FilterDataArray(RealType *outs,
                                                const RealType *data,
                                                unsigned int ln, RealType &sV0,
                                                RealType &sV1, RealType &sV2) {
  /**
...
//
template <typename TInputImage, typename TOutputImage>
void RecursiveLineYvvGaussianImageFilter<TInputImage, TOutputImage>::
...
void RecursiveLineYvvGaussianImageFilter<
  ...
  if (ln < 4) {
    itkExceptionMacro("The number of pixels along direction "
                      << this->m_Direction
                      << " is less than 4. This filter requires a minimum of "
                         "four pixels along the dimension to be processed.");
  }
}

template <typename TInputImage, typename TOutputImage>
void RecursiveLineYvvGaussianImageFilter<TInputImage,
...
template <typename TInputImage, typename TOutputImage>
void RecursiveLineYvvGaussianImageFilter<TInputImage, TOutputImage>::PrintSelf(
