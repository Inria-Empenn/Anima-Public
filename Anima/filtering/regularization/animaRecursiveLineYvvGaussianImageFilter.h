...
protected:
  ...
  template <class T>
  inline void ComputeCausalPart(itk::VariableLengthVector<T> &out,
  ...
  /** Utility function to compute causal part for ITK vector type */
  template <class T>
  inline void ComputeAntiCausalPart(itk::VariableLengthVector<T> &out,
  ...
  inline void ComputeCausalBase(const T &data, T &V0, T &V1, T &V2) {
    V0 = data / (1.0 - m_B1 - m_B2 - m_B3);
    V1 = V0;
    V2 = V1;
  }

  template <class T>
  inline void ComputeCausalBase(const itk::VariableLengthVector<T> &data,
                                itk::VariableLengthVector<T> &V0,
                                itk::VariableLengthVector<T> &V1,
                                itk::VariableLengthVector<T> &V2) {
    unsigned int vSize = data.GetSize();
    ...
    for (unsigned int i = 0; i < vSize; ++i) {
      V0[i] = data[i] / (1.0 - m_B1 - m_B2 - m_B3);
      V1[i] = V0[i];
      V2[i] = V1[i];
    }
  }
  ...
  template <class T>
  inline void ComputeAntiCausalBase(const T &data, T *outs, T &V0, T &V1, T &V2,
                                    unsigned int ln) {
    ...
    // This was not in the 2006 Triggs paper but sounds quite logical since m_B
    // is not one
    V0 *= m_B;
    V1 *= m_B;
    V2 *= m_B;
  ...
  template <class T>
  inline void ComputeAntiCausalBase(const itk::VariableLengthVector<T> &data,
