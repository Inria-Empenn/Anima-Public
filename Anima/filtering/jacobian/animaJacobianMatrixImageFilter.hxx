...
void JacobianMatrixImageFilter<TPixelType, TOutputPixelType,
                               Dimension>::BeforeThreadedGenerateData() {
...
template <typename TPixelType, typename TOutputPixelType,
          unsigned int Dimension>
void JacobianMatrixImageFilter<TPixelType, TOutputPixelType,
                               Dimension>::SetNeighborhood(unsigned int val) {
  if (val == 0) {
    m_Neighborhood = 1;
...
template <typename TPixelType, typename TOutputPixelType,
          unsigned int Dimension>
void JacobianMatrixImageFilter<TPixelType, TOutputPixelType, Dimension>::
  ...
  while (!inputItr.IsAtEnd()) {
    ...
    for (unsigned int i = 0; i < Dimension; ++i) {
      ...
      unsigned int upperIndex =
          std::min(maxValue, (unsigned int)(currentIndex[i] + m_Neighborhood));
    ...
    while (!internalItr.IsAtEnd()) {
      ...
      if (m_OnlySixConnectivity) {
        bool faceConnex =
            this->CheckFaceConnectivity(internalIndex, currentIndex);
        if (!faceConnex) {
          ++internalItr;
          continue;
        }
      ...
      for (unsigned int i = 0; i < indexesUsedVector.size(); ++i) {
        if ((internalIndexOpposite == indexesUsedVector[i]) ||
            (internalIndex == indexesUsedVector[i])) {
          skipVoxel = true;
          break;
        }
      }
      ...
      pixelAfterValue =
          m_FieldInterpolator->EvaluateAtIndex(internalIndexOpposite);

      this->GetInput()->TransformIndexToPhysicalPoint(internalIndex,
                                                      pointBefore);
      this->GetInput()->TransformIndexToPhysicalPoint(internalIndexOpposite,
                                                      pointAfter);
      ...
      double dataWeight =
          std::sqrt(std::exp(-pointDist / (meanSpacing * meanSpacing)));
      ...
      for (unsigned int i = 0; i < Dimension; ++i) {
        double diffValue =
            (pixelAfterValue[i] - pixelBeforeValue[i]) / pointDist;
        dataVector[pos * Dimension + i] = diffValue * dataWeight;

        for (unsigned int j = 0; j < Dimension; ++j)
          dataMatrix(pos * Dimension + i, i * Dimension + j) =
              unitVector[j] * dataWeight;
      }
...
template <typename TPixelType, typename TOutputPixelType,
          unsigned int Dimension>
bool JacobianMatrixImageFilter<TPixelType, TOutputPixelType, Dimension>::
    CheckFaceConnectivity(const IndexType &internalIndex,
