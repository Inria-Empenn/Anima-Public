...
/**
 * @brief MultiCompartmentModel: holds several diffusion compartments, ordered
 * by type It also handles weights of the different compartments. Although there
 * are N weights in memory for N compartments, the parameters returned and set
 * are considered to have one less weight in them, the first one being removed.
 * This comes from the fact that they sum up to 1.
 */
...
public:
  ...
  /**
   * Returns a clone of this MCM in terms of compartment organization, used by
   * Clone method Does not copy parameters of each compartment though, neither
   * weights
   */
  ...
  ListType &GetSignalJacobian(double smallDelta, double bigDelta,
                              double gradientStrength,
                              const Vector3DType &gradient);
  ...
  //! Re-orient the MCM using the provided transform, the flag precises if the
  //! transform id rigid or affine
