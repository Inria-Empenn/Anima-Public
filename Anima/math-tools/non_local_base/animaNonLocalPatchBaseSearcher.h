...
/**
 * Abstract class for non local patch matching. May be used to search in
 * multiple images if the concrete class does support it. Does not compute
 * weights and samples for central voxels as these would always get a weight
 * of 1. Instead, it is the developer task to implement it in his filter
 */
template <class ImageType> class NonLocalPatchBaseSearcher {
public:
...
protected:
  ...
  virtual void ComputeComparisonProperties(unsigned int index,
                                           ImageRegionType &movingPatch) {}
  virtual double ComputeWeightValue(unsigned int index,
                                    ImageRegionType &refPatch,
                                    ImageRegionType &movingPatch) = 0;
  virtual bool TestPatchConformity(unsigned int index,
