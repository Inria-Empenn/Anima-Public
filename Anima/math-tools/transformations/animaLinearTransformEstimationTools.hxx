...
void computeTranslationLSWFromTranslations(
    std::vector<itk::Point<TInput, NDimensions>> &inputOrigins,
...
template <class TInput, class TScalarType, unsigned int NDimensions>
void computeRigidLSWFromTranslations(
    std::vector<itk::Point<TInput, NDimensions>> &inputOrigins,
    std::vector<itk::Point<TInput, NDimensions>> &inputTransformed,
    std::vector<TInput> &weights,
    typename itk::AffineTransform<TScalarType, NDimensions>::Pointer
  ...
  itk::SymmetricEigenAnalysis<vnl_matrix<TInput>, vnl_diag_matrix<TInput>,
                              vnl_matrix<TInput>>
      eigenSystem(4);
  ...
  vnl_matrix<TScalarType> rotationMatrix =
      anima::computeRotationFromQuaternion<TInput, TScalarType>(
          eVec.get_row(0));
...
template <class TInput, class TScalarType, unsigned int NDimensions>
itk::Point<TInput, NDimensions> computeAnisotropSimLSWFromTranslations(
    std::vector<itk::Point<TInput, NDimensions>> &inputOrigins,
    std::vector<itk::Point<TInput, NDimensions>> &inputTransformed,
    std::vector<TInput> &weights,
    typename itk::AffineTransform<TScalarType, NDimensions>::Pointer
  ...
  while (((diffRot > eps) || (diffScal > eps)) && (iter < 100)) {
    ...
    for (unsigned int i = 0; i < nbPts; ++i) {
      xiVector = scal * UMatrix.transpose() *
                 (inputOrigins[i].GetVnlVector() - barX.GetVnlVector());
    ...
    for (unsigned int i = 0; i < nbPts; ++i) {
      yVector = inputTransformed[i].GetVnlVector() - barY.GetVnlVector();
      xVector = UMatrix.transpose() *
                (inputOrigins[i].GetVnlVector() - barX.GetVnlVector());
      for (unsigned int j = 0; j < NDimensions; ++j) {
        anima::pairingToQuaternionScalingsDerivative(xVector, yVector,
                                                     tmpMatrix, j);
  ...
  vnl_matrix<TScalarType> rotationMatrix =
      anima::computeRotationFromQuaternion<TInput, TScalarType>(q);
  vnl_matrix<TScalarType> linearPartMatrix =
      rotationMatrix * scal * UMatrix.transpose();
...
template <class TInput, class TScalarType, unsigned int NDimensions>
void computeLogEuclideanAverage(
    std::vector<vnl_matrix<TInput>> &inputTransforms,
    std::vector<TInput> &weights,
    typename itk::AffineTransform<TScalarType, NDimensions>::Pointer
        &resultTransform) {
...
template <class TInput, class TScalarType, unsigned int NDimensions>
itk::Point<TInput, NDimensions> computeAffineLSWFromTranslations(
    std::vector<itk::Point<TInput, NDimensions>> &inputOrigins,
    std::vector<itk::Point<TInput, NDimensions>> &inputTransformed,
    std::vector<TInput> &weights,
    typename itk::AffineTransform<TScalarType, NDimensions>::Pointer
  ...
  for (unsigned int i = 0; i < nbPts; ++i) {
    for (unsigned int j = 0; j < NDimensions; ++j)
      for (unsigned int k = 0; k < NDimensions; ++k) {
        SigmaXXMatrix(j, k) += weights[i] * (inputOrigins[i][j] - barX[j]) *
                               (inputOrigins[i][k] - barX[k]);
        SigmaYXMatrix(j, k) += weights[i] * (inputTransformed[i][j] - barY[j]) *
                               (inputOrigins[i][k] - barX[k]);
      }
  }
...
vnl_matrix<TOutput>
computeRotationFromQuaternion(vnl_vector<TInput> eigenVector) {
  double normVector = 0;
  ...
  resVal(0, 0) =
      (1.0 / normVector) *
      (eigenVector[0] * eigenVector[0] + eigenVector[1] * eigenVector[1] -
       eigenVector[2] * eigenVector[2] - eigenVector[3] * eigenVector[3]);
  resVal(0, 1) = (2.0 / normVector) * (eigenVector[1] * eigenVector[2] -
                                       eigenVector[0] * eigenVector[3]);
  resVal(0, 2) = (2.0 / normVector) * (eigenVector[1] * eigenVector[3] +
                                       eigenVector[0] * eigenVector[2]);

  resVal(1, 0) = (2.0 / normVector) * (eigenVector[1] * eigenVector[2] +
                                       eigenVector[0] * eigenVector[3]);
  resVal(1, 1) =
      (1.0 / normVector) *
      (eigenVector[0] * eigenVector[0] - eigenVector[1] * eigenVector[1] +
       eigenVector[2] * eigenVector[2] - eigenVector[3] * eigenVector[3]);
  resVal(1, 2) = (2.0 / normVector) * (eigenVector[2] * eigenVector[3] -
                                       eigenVector[0] * eigenVector[1]);

  resVal(2, 0) = (2.0 / normVector) * (eigenVector[1] * eigenVector[3] -
                                       eigenVector[0] * eigenVector[2]);
  resVal(2, 1) = (2.0 / normVector) * (eigenVector[2] * eigenVector[3] +
                                       eigenVector[0] * eigenVector[1]);
  resVal(2, 2) =
      (1.0 / normVector) *
      (eigenVector[0] * eigenVector[0] - eigenVector[1] * eigenVector[1] -
...
void pairingToQuaternion(
  ...
  return pairingToQuaternion(inputPoint, inputTransformedPoint, outputMatrix,
                             NDimensions);
}
...
void pairingToQuaternion(const PointType &inputPoint,
                         const PointType &inputTransformedPoint,
                         vnl_matrix<TOutput> &outputMatrix, unsigned int ndim) {
...
template <class TInput, class TOutput, unsigned int NDimensions>
void pairingToQuaternionScalingsDerivative(
    const vnl_vector_fixed<TInput, NDimensions> &inputPoint,
    const vnl_vector_fixed<TInput, NDimensions> &inputTransformedPoint,
    vnl_matrix<TOutput> &outputMatrix, const int &dimScal) {
  return pairingToQuaternionScalingsDerivative(
      inputPoint, inputTransformedPoint, outputMatrix, NDimensions, dimScal);
}
...
template <class PointType, class TOutput>
void pairingToQuaternionScalingsDerivative(
    const PointType &inputPoint, const PointType &inputTransformedPoint,
    vnl_matrix<TOutput> &outputMatrix, unsigned int ndim, const int &dimScal) {
  outputMatrix.set_size(ndim + 1, ndim + 1);
  outputMatrix.fill(0);
