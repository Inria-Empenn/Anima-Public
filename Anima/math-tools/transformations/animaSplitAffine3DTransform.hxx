...
SplitAffine3DTransform<TScalarType>::SplitAffine3DTransform()
...
// Set Parameters
template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::SetParameters(
...
SplitAffine3DTransform<TScalarType>::GetParameters(void) const {
...
template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::SetIdentity() {
  m_FirstAngle.Fill(itk::NumericTraits<TScalarType>::Zero);
  m_LogScale.Fill(itk::NumericTraits<TScalarType>::Zero);
  m_SecondAngle.Fill(itk::NumericTraits<TScalarType>::Zero);
...
template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::SetFirstAngle(
    const VectorType &angle) {
  m_FirstAngle = angle;
  this->ComputeMatrix();
  this->ComputeOffset();
}
...
template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::SetLogScale(const VectorType &scale) {
  m_LogScale = scale;
  this->ComputeMatrix();
  this->ComputeOffset();
}
...
template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::SetSecondAngle(
    const VectorType &angle) {
  m_SecondAngle = angle;
  this->ComputeMatrix();
  this->ComputeOffset();
...
// Compute the matrix
template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::ComputeMatrix() {
  ...
  vnl_matrix<TScalarType> resultMatrix =
      firstRotationMatrix * scaleMatrix * secondRotationMatrix;
  MatrixType varMatrix;
  for (unsigned int i = 0; i < InputSpaceDimension; ++i)
    for (unsigned int j = 0; j < InputSpaceDimension; ++j)
      varMatrix(i, j) = resultMatrix(i, j);
  this->SetVarMatrix(varMatrix);
}

template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::ComputeMatrixParameters() {
  itkExceptionMacro(<< "Setting the matrix of a SplitAffine3D transform is not "
                       "supported at this time.");
}

// Print self
template <class TScalarType>
void SplitAffine3DTransform<TScalarType>::PrintSelf(std::ostream &os,
