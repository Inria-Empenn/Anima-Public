...
void TRKReader::Update() {
  ...
  if (!inFile.is_open())
    throw itk::ExceptionObject(
        __FILE__, __LINE__, "Unable to open file " + m_FileName, ITK_LOCATION);

  inFile.read((char *)&headerStr, sizeof(anima::TRKHeaderStructure));

  if (headerStr.version != 2)
    throw itk::ExceptionObject(
        __FILE__, __LINE__, "TRK reader only supports version 2", ITK_LOCATION);
  ...
  std::vector<vtkSmartPointer<vtkDoubleArray>> scalarArrays(
      headerStr.n_scalars);
  ...
  std::vector<vtkSmartPointer<vtkDoubleArray>> cellArrays(
      headerStr.n_properties);
  ...
  for (unsigned int i = 0; i < nCells; ++i) {
    ...
    for (unsigned int j = 0; j < npts; ++j) {
      inFile.read((char *)pointValues.data(),
                  (3 + headerStr.n_scalars) * sizeof(float));
    ...
    inFile.read((char *)cellScalars.data(),
                headerStr.n_properties * sizeof(float));
