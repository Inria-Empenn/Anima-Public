...
void TRKWriter::Update() {
  ...
  unsigned int numScalarFields =
      m_TrackData->GetPointData()->GetNumberOfArrays();
  unsigned int numPropertyFields =
      m_TrackData->GetCellData()->GetNumberOfArrays();

  unsigned int pos = 0;
  std::vector<vtkDoubleArray *> scalarArrays;
  for (unsigned int i = 0; i < numScalarFields; ++i) {
    unsigned int numComponents =
        m_TrackData->GetPointData()->GetArray(i)->GetNumberOfComponents();
    if (numComponents != 1)
      continue;
    ...
    ++pos;
    scalarArrays.push_back(dynamic_cast<vtkDoubleArray *>(
        m_TrackData->GetPointData()->GetArray(i)));
  ...
  for (unsigned int i = 0; i < numPropertyFields; ++i) {
    unsigned int numComponents =
        m_TrackData->GetCellData()->GetArray(i)->GetNumberOfComponents();
    if (numComponents != 1)
      continue;
    ...
    propertyArrays.push_back(dynamic_cast<vtkDoubleArray *>(
        m_TrackData->GetCellData()->GetArray(i)));
    ++pos;
    if (pos == 10)
      break;
  }
  ...
  // http://www.tractometer.org/ismrm_2015_challenge/tracts_file_specs is the
  // ref but many options exist
  itk::SpatialOrientationAdapter orientationAdapter;
  itk::SpatialOrientationAdapter::OrientationType orientationType =
      orientationAdapter.FromDirectionCosines(direction);

  itk::SpatialOrientation::CoordinateTerms testOrientation =
      (itk::SpatialOrientation::CoordinateTerms)(
          static_cast<unsigned int>(orientationType) >>
  ...
  truncatedOrientationType = (itk::SpatialOrientationAdapter::OrientationType)(
      static_cast<unsigned int>(orientationType) -
      (static_cast<unsigned int>(testOrientation) << static_cast<unsigned int>(
           itk::SpatialOrientation::ITK_COORDINATE_TertiaryMinor)));
  testOrientation = (itk::SpatialOrientation::CoordinateTerms)(
      static_cast<unsigned int>(truncatedOrientationType) >>
      static_cast<unsigned int>(
  ...
  if (!outFile.is_open())
    throw itk::ExceptionObject(
        __FILE__, __LINE__, "Unable to op en file " + m_FileName, ITK_LOCATION);
  ...
  for (unsigned int i = 0; i < headerStr.n_count; ++i) {
    ...
    for (unsigned int j = 0; j < cellSize; ++j) {
      ...
      double *point =
          m_TrackData->GetPoint(m_TrackData->GetCell(i)->GetPointId(j));
      ...
      outFile.write((char *)cellData.data(),
                    (numScalarFields + 3) * sizeof(float));
    }
    ...
    if (numPropertyFields > 0) {
      ...
      outFile.write((char *)cellScalars.data(),
                    numPropertyFields * sizeof(float));
    }
  }
