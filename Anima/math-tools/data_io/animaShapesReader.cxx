...
void ShapesReader::ReadFileAsMedinriaFibers() {
  ...
  if (loadOk != tinyxml2::XML_SUCCESS)
    throw itk::ExceptionObject(
        __FILE__, __LINE__, "Error reading XML FDS file header", ITK_LOCATION);

  tinyxml2::XMLElement *vtkFileNode = doc.FirstChildElement("VTKFile");
  if (!vtkFileNode)
    throw itk::ExceptionObject(__FILE__, __LINE__, "Malformed FDS file",
                               ITK_LOCATION);
  ...
  if (extensionName == "vtk") {
    ...
    vtkSmartPointer<vtkXMLPolyDataReader> vtkReader =
    ...
    throw itk::ExceptionObject(__FILE__, __LINE__,
                               "Unsupported fibers extension inside FDS files.",
                               ITK_LOCATION);
}
...
void ShapesReader::ReadFileAsCSV() {
  std::string extension =
      vtksys::SystemTools::GetFilenameLastExtension(m_FileName);
  ...
  if (numberOfColumns < 5)
    throw itk::ExceptionObject(__FILE__, __LINE__,
                               "The CSV should contain at least 5 columns.",
                               ITK_LOCATION);
  ...
  if (header[0] != "X" || header[1] != "Y" || header[2] != "Z" ||
    ...
    throw itk::ExceptionObject(
        __FILE__, __LINE__,
        "The CSV should contain at least the following first 5 variables in "
        "order: X, Y, Z, PointId, StreamlineId.",
        ITK_LOCATION);
