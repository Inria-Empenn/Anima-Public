...
public:
  ...
  using Self = BLMLambdaCostFunction;
  ...
  void SetInversePivotVector(std::vector<unsigned int> &invPiv) {
    m_InversePivotVector = invPiv;
  }
  void SetPivotVector(std::vector<unsigned int> &piv) { m_PivotVector = piv; }
  void SetLowerBoundsPermutted(ParametersType &lb) {
    m_LowerBoundsPermutted = lb;
  }
  void SetUpperBoundsPermutted(ParametersType &ub) {
    m_UpperBoundsPermutted = ub;
  }
  void SetPreviousParametersPermutted(ParametersType &val) {
    m_PreviousParametersPermutted = val;
  }

  void SetInputWorkMatricesAndVectorsFromQRDerivative(
      vnl_matrix<double> &qrDerivative, ParametersType &qtResiduals,
      unsigned int rank);
