 ...
 * Benoit Scherrer, CRL (Computational Radiology Laboratory), Harvard Medical
 *School
 *
 * \section Example
 ...
 *		itk::MyCostFunction::Pointer costFun =
 *itk::MyCostFunction::New();
 *
 *		// Create the optimizer
 *		itk::NLOPTOptimizers::Pointer optimizer =
 *itk::NLOPTOptimizers::New(); optimizer->SetCostFunction(costFun);
 *
 *		// Set the parameters
...
public:
  using Self = NLOPTOptimizers; /**< Standard ITK "Self" typedef. */
  using Superclass =
      SingleValuedNonLinearOptimizer; /**< Standard ITK "Superclass" typedef. */
  using Pointer =
      itk::SmartPointer<Self>; /**< Standard ITK "Pointer" typedef. */
  using ConstPointer =
      itk::SmartPointer<const Self>; /**< Standard ITK "ConstPointer" typedef.
                                      */

  using ParametersType =
      SingleValuedNonLinearOptimizer::ParametersType; /**< Parameter type */
  using ConstraintsFunctionType = anima::NLOPTParametersConstraintFunction; /**<
  ...
  using CostFunctionType = itk::SingleValuedCostFunction;
   ...
   * less than tol multiplied by the absolute value of the function value. (If
   * there is any chance that your optimum function value is close to zero, you
   * might want to set an absolute tolerance with nlopt_set_ftol_abs as well.)
   ...
   * by less than tol multiplied by the absolute value of the parameter. (If
   * there is any chance that an optimal parameter is close to zero, you might
   * want to set an absolute tolerance with nlopt_set_xtol_abs as well.)
   * Criterion is disabled if tol is non-positive.
   *
   * \note Multiple stopping criteria for the optimization are
   * supported. The optimization halts whenever any one of these criteria is
   ...
   * algorithm and on how slow your function evaluation is.) Criterion is
   * disabled if maxtime is non-positive
   *
   * \note Multiple stopping criteria for the optimization are
   * supported. The optimization halts whenever any one of these criteria is
   * satisfied. In some cases, the precise interpretation of the stopping
   ...
   * increasing it increases the memory requirements but speeds convergence.*/
  itkSetMacro(VectorStorageSize, int);
  itkGetConstReferenceMacro(VectorStorageSize, int);

  /** Stochastic population size:
   * This is for random global optimization. It sets the number of random
   * particles to propagate from.*/
  ...
  /** Tells Nlopt to stop the optimization at the next iteration and to returns
...
protected:
  ...
  static double NloptFunctionWrapper(unsigned n, const double *x, double *grad,
                                     void *data);
