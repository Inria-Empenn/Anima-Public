 ...
 * Constructor
 */
VoxelExhaustiveOptimizer ::VoxelExhaustiveOptimizer() {
...
const VoxelExhaustiveOptimizer::MeasureType &
VoxelExhaustiveOptimizer ::GetCurrentCost() const {
  if (this->m_Maximize)
...
void VoxelExhaustiveOptimizer ::StartWalking(void) {
  ...
  if (scales.size() != spaceDimension) {
    itkExceptionMacro(<< "The size of Scales is " << scales.size()
                      << ", but the NumberOfParameters is " << spaceDimension
                      << ".");
  }

  if (m_Geometry.rows() != spaceDimension) {
    itkExceptionMacro(<< "The size of the geometry matrix is "
                      << m_Geometry.rows() << ", but the NumberOfParameters is "
                      << spaceDimension << ".");
  }
  ...
  for (unsigned int i = 0; i < spaceDimension; i++)
    position[i] =
        this->GetInitialPosition()[i] - m_NumberOfSteps[i] * scales[i];
 ...
 */
void VoxelExhaustiveOptimizer ::ResumeWalking(void) {
  m_Stop = false;
  ...
  while (!m_Stop) {
    ...
    m_StopConditionDescription.str("");
    m_StopConditionDescription << this->GetNameOfClass() << ": Running. ";
    m_StopConditionDescription << "@ index " << this->GetCurrentIndex()
                               << " value is " << this->GetCurrentValue();
...
void VoxelExhaustiveOptimizer ::StopWalking(void) {
  m_Stop = true;
  this->InvokeEvent(itk::EndEvent());
}

void VoxelExhaustiveOptimizer ::IncrementIndex(ParametersType &newPosition) {
  unsigned int idx = 0;
  ...
  for (unsigned int i = 0; i < spaceDimension; i++)
    newPosition[i] =
        (m_CurrentIndex[i] - m_NumberOfSteps[i]) * this->GetScales()[i] +
...
void VoxelExhaustiveOptimizer ::PrintSelf(std::ostream &os,
  ...
  os << indent
     << "MinimumMetricValuePosition = " << m_MinimumMetricValuePosition
     << std::endl;
  os << indent
     << "MaximumMetricValuePosition = " << m_MaximumMetricValuePosition
     << std::endl;
}
