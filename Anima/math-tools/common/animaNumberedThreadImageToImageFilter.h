...
/**
 * @brief Implements a class to handle thread number in a dynamic way for
 * multithreaded methods needing thread numbering even for dynamic threading
 */
...
public:
  ...
  using Self = NumberedThreadImageToImageFilter;
...
protected:
  ...
  void
  InitializeZeroPixel(TOutputImage *image,
  ...
  void InitializeZeroPixel(TOutputImage *itkNotUsed(image),
                           PixelType &zeroPixel) {
...
private:
  ...
  // Optimization of multithread code, compute only on region defined from
  // mask... Uninitialized in constructor.
