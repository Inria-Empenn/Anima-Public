...
/**
 * @brief Class computing the 3-class GMM respresenting the NABT, where each
 * Gaussian represents one of the brain tissues WM, GM and CSF. First a model
 * initializer is launched, then the REM algorithm is performed using this
 * initialization. The NABT model can be written in a csv file.
 */
template <typename TInputImage, typename TMaskImage,
          typename TAtlasImage = TInputImage>
class ComputeSolution : public itk::ProcessObject {
public:
  /** Standard class typedefs. */
  using Self = ComputeSolution;
  ...
  using ImagePointerUC = ImageTypeUC::Pointer;

  using AtlasInitializerType =
      AtlasInitializer<ImageTypeUC, MaskImageType, TAtlasImage>;
  using HierarchicalType = HierarchicalInitializer<ImageTypeUC, MaskImageType>;
  using GaussianREMEstimatorType =
      GaussianREMEstimator<ImageTypeUC, MaskImageType>;
  using GaussianEMEstimatorType =
      GaussianEMEstimator<ImageTypeUC, MaskImageType>;

  /** Define filter types. */
  using RescaleFilterType =
      typename itk::RescaleIntensityImageFilter<TInputImage, ImageTypeUC>;

  using NumericType = double;
  using MeasurementVectorType = itk::VariableLengthVector<NumericType>;
  ...
  void SetSolutionWriteFilename(std::string filename) {
    m_SolutionWriteFilename = filename;
  }
  void SetSolutionReadFilename(std::string filename) {
    m_SolutionReadFilename = filename;
  }
  ...
  int PrintSolution(std::vector<double> alphas,
