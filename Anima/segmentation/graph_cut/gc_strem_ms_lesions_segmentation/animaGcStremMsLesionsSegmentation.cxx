...
int main(int argc, const char** argv)
{
    ...
    TCLAP::ValueArg<double> thStremCSFArg("","th-stremCSF","Strem threshold in CSF image (default: 0.5)",false,0.5,"strem threshold CSF",cmd);
    TCLAP::ValueArg<double> thStremGMArg("","th-stremGM","Strem threshold in GM image (default: 0.5)",false,0.5,"strem threshold GM",cmd);
    TCLAP::ValueArg<double> thStremWMArg("","th-stremWM","Strem threshold in WM image (default: 0.5)",false,0.5,"strem threshold WM",cmd);
    ...
    TCLAP::ValueArg<double> multiVarSourcesArg("","mv","Coefficient to multiply the variance value of the source seeds (default: 1)",false,1,"sources multiply variance",cmd);
    TCLAP::ValueArg<double> multiVarSinksArg("","ms","Coefficient to multiply the variance value of the sink seeds (default: 1)",false,1,"sinks multiply variance",cmd);
    TCLAP::ValueArg<double> sigmaArg("","sigma","Sigma value (default: 0.6)",false,0.6,"sigma",cmd);
    TCLAP::ValueArg<double> alphaArg("","alpha","Alpha value (default: 10)",false,10,"alpha",cmd);
    TCLAP::ValueArg<std::string> matrixGradArg("","mat","Spectral gradient matrix file",false,"","spectral gradient matrix file",cmd);

    // Heuristic rules
    TCLAP::ValueArg<double> minLesionSizeArg("","ml","Minimum lesion size in mm3 (default: 0)",false,0,"minimum lesion size",cmd);
    ...
    TCLAP::ValueArg<double> thWMmapArg("","th-wm-map","Probability threshold to define white matter map (default: 0.2)",false,0.2,"threshold white matter map",cmd);
    TCLAP::ValueArg<double> contourWMRatioArg("","WMratio","Minimum percentage of WM that must surround lesions",false,0,"WM ratio contour",cmd);
    ...
    {
        segFilter->SetInputCSFAtlas( anima::readImage<InputImageTypeD>( atlasFileCSFArg.getValue() ) );
    }

    if( atlasFileGMArg.getValue()!="" )
    {
        segFilter->SetInputGMAtlas( anima::readImage<InputImageTypeD>( atlasFileGMArg.getValue() ) );
    }
    ...
    {
        segFilter->SetMask( anima::readImage<InputImageTypeD>( maskFileArg.getValue() ) );
    }
