...
enum LesionSegmentationType { strem = 0, gcem, gcemAndManualGC, manualGC };

enum InitializationType { Atlas = 0, HierarchicalDP, HierarchicalFLAIR };
 ...
 *  The NABT model computation requires strictly 3 sequences: T1-w and two
 * sequences among T2-w, DP and FLAIR.
 *
 *  2) Detection of candidate lesions:
 *  The lesion detection can be done two different ways:
 *  - applying a simple threshold after computing the mahalanobis distances on
 * each one of the healthy tissues. (LesionSegmentationType == strem)
 *  - applying a graph cut segmentation. (LesionSegmentationType == gcem)
 *    The automatic graph cut segmentation requires a sources and a sink
 * probabilities maps that are computed from the mahalanobis images. The sources
 * map is computed using also fuzzy weights between 0 and 1, based on T2-w, DP
 * and FLAIR hyper-intensities. Additionally, binary sources and sinks masks can
 * be added as entries of the graph cut.(LesionSegmentationType ==
 * gcemAndManualGC) Their information will be mixed with the automatic sources
 * and sinks probabilities map. This option simulates the lesion segmentation
 * correction done by a user that adds/removes seeds for the graph cut
 * computation.
 *
 *    The last segmentation type option (LesionSegmentationType == manualGC)
 * just uses manual binary sources and sinks masks as entries of the graph cut.
 *    This option does not perform the estimation of the NABT model.
 *
 *  3) Application of a priori heuristic rules to extract the MS lesions from
 * others outliers: The heuritic rules that can be used to refine the
 * segmentation are:
 ...
 *  - hyper-intensity rule to remove voxels that are not hyper-intense comparing
 * to the white matter in T2, DP and FLAIR sequence.
 *  - white matter neighbor rule to remove lesions that are not in white matter.
 *
 *
 * Additionally to the lesions segmentation, the filter provides a segmentation
 * of the healthy tissues based on the NABT estimation.
 *
 */
...
public:
  ...
  using Self = GcStremMsLesionsSegmentationFilter;
  ...
  using ImageIteratorTypeConstInput =
  ...
  using ImageTypeUC = itk::Image<PixelTypeUC, 3>;
  using ImageIteratorTypeUC = itk::ImageRegionIterator<ImageTypeUC>;
  using ImageIteratorTypeConstUC = itk::ImageRegionConstIterator<ImageTypeUC>;

  using PixelTypeD = double;
  using ImageTypeD = itk::Image<PixelTypeD, 3>;
  using ImageIteratorTypeD = itk::ImageRegionIterator<ImageTypeD>;
  ...
  /**  Define filters. */
  using GraphCutFilterType = anima::GraphCutFilter<TInputImage, ImageTypeUC>;
  using CheckStructureNeighborFilterFilterType =
      anima::CheckStructureNeighborFilter<ImageTypeUC, ImageTypeUC>;
  using RemoveTouchingBorderFilterType =
      anima::RemoveTouchingBorderFilter<ImageTypeUC, ImageTypeUC, ImageTypeUC>;
  using TLinksFilterType = anima::TLinksFilter<TInputImage, ImageTypeD>;
  using ComputeMahalanobisImagesFilterType =
      anima::ComputeMahalanobisImagesFilter<ImageTypeUC, ImageTypeUC,
                                            ImageTypeD>;
  using ComputeSolutionType =
      anima::ComputeSolution<ImageTypeUC, ImageTypeUC, ImageTypeD>;

  using RescaleFilterType =
      itk::RescaleIntensityImageFilter<TInputImage, ImageTypeUC>;
  using MaximumFilterType =
      itk::MaximumImageFilter<ImageTypeD, ImageTypeD, ImageTypeD>;
  using MinimumFilterTypeF =
      itk::MinimumImageFilter<ImageTypeD, ImageTypeD, ImageTypeD>;
  using MinimumFilterTypeUC =
      itk::MinimumImageFilter<ImageTypeUC, ImageTypeUC, ImageTypeUC>;
  using BinaryThresholdImageFilterType_F_UC =
      itk::BinaryThresholdImageFilter<ImageTypeD, ImageTypeUC>;
  using BinaryThresholdImageFilterType_UC_UC =
      itk::BinaryThresholdImageFilter<ImageTypeUC, ImageTypeUC>;
  using IntensityWindowingImageFilterType =
      itk::IntensityWindowingImageFilter<ImageTypeUC, ImageTypeD>;
  using MaskFilterType_UC_UC = itk::MaskImageFilter<ImageTypeUC, ImageTypeUC>;
  using MaskFilterType_F_UC = itk::MaskImageFilter<ImageTypeD, ImageTypeUC>;
  using ConnectedComponentType =
      itk::ConnectedComponentImageFilter<ImageTypeUC, ImageTypeInt>;
  using RelabelComponentType =
      itk::RelabelComponentImageFilter<ImageTypeInt, ImageTypeInt>;
  ...
  LesionSegmentationType GetLesionSegmentationType() {
    return m_LesionSegmentationType;
  }
  void SetLesionSegmentationType(LesionSegmentationType type) {
    m_LesionSegmentationType = type;
  }
...
private:
  ...
  unsigned int m_IndexImageT1, m_IndexImageT2, m_IndexImageDP,
      m_IndexImageFLAIR, m_IndexImageT1Gd, m_IndexMask;
  unsigned int m_IndexAtlasCSF, m_IndexAtlasGM, m_IndexAtlasWM,
      m_IndexLesionPrior;
  unsigned int m_IndexSourcesProba, m_IndexSinksProba, m_IndexSourcesMask,
      m_IndexSinksMask;
  ...
  double m_RejRatioHierar; /*!< Rejection ratio of the EM used in Hierarchical
                              intialisation */
  ...
  int m_EmIter_concentration; /*!< number of iterations for EM algorithm between
                                 each concentration step if REME */
  bool m_EM_before_concentration; /*!< Process EM before first concentration
                                     step */

  double m_MahalanobisThCSF; /*!< Outliers Selection, mahalanobis distance for
                                CSF */
  ...
  double m_LesionPriorProportion; /*! Lesion prior proportion in graph cut
                                     algorithm */
