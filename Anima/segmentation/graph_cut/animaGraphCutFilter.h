 ...
 *  First the sources and sinks probabilities maps are computed using the
 * TLinkFilter. Then the Graph3DFilter is called to perform the graph cut.
 */
template <typename TInput, typename TOutput>
class GraphCutFilter : public itk::ImageToImageFilter<TInput, TOutput> {
public:
  ...
  /** probabilities of the object and the background, respectively or binary
   * mask of the object and the background, respectively
   */
...
protected:
  using Graph3DFilterType = Graph3DFilter<TInput, TOutput>;
  using TLinksFilterType = TLinksFilter<TInput, TSeedProba>;
  ...
  GraphCutFilter() {
    ...
    m_NbInputs = 2;
    m_NbMaxImages = 12;
    m_IndexSourcesMask = m_NbMaxImages, m_IndexSourcesProba = m_NbMaxImages,
    m_IndexSinksMask = m_NbMaxImages, m_IndexSinksProba = m_NbMaxImages,
    m_IndexMask = m_NbMaxImages;
    ...
    m_Graph3DFilter->SetInputSeedProbaSources(
        m_TLinksFilter->GetOutputSources());
