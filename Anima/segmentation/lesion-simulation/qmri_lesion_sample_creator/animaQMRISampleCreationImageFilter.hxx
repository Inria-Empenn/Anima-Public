...
template <class TInputImage, class TOutputImage> const double QMRISampleCreationImageFilter<TInputImage, TOutputImage>::m_LesionSizeAFactor = 0.0253;
template <class TInputImage, class TOutputImage> const double QMRISampleCreationImageFilter<TInputImage, TOutputImage>::m_LesionSizeBFactor = - 0.3506;
...
void
QMRISampleCreationImageFilter<TInputImage, TOutputImage>
::AddQMRIVarianceImage(TInputImage *varImage)
{
...
template <class TInputImage, class TOutputImage>
void QMRISampleCreationImageFilter<TInputImage, TOutputImage>
::InitializeOutputs()
{
...
::ReadQMRILesionRelationships()
{
    ...
    vnl_matrix <double> eVecs(this->GetNumberOfIndexedInputs(),this->GetNumberOfIndexedInputs());
    vnl_diag_matrix <double> eVals(this->GetNumberOfIndexedInputs());
    itk::SymmetricEigenAnalysis < vnl_matrix <double>, vnl_diag_matrix<double>, vnl_matrix <double> > eigenComputer(this->GetNumberOfIndexedInputs());
    eigenComputer.ComputeEigenValuesAndVectors(m_QMRILesionCovarianceRelationship, eVals, eVecs);

    for (unsigned int i = 0;i < this->GetNumberOfIndexedInputs();++i)
        eVals[i] = sqrt(eVals[i]);

    anima::RecomposeTensor(eVals,eVecs,m_QMRILesionCovarianceRelationship);
}

template <class TInputImage, class TOutputImage>
void
QMRISampleCreationImageFilter<TInputImage, TOutputImage>
::ReadLesionSizesDistributions(std::string sizeDistributionFile)
{
    m_XAxisLesionSizesDistribution.clear();
    m_YAxisLesionSizesDistribution.clear();
...
::UpdateQMRIOnLesions()
{
    ...
    while (!validFactors)
    {
        anima::SampleFromMultivariateGaussianDistribution(m_QMRILesionMeanRelationships,m_QMRILesionCovarianceRelationship,
                                                                          multiplyingFactors,m_Generator,false);
