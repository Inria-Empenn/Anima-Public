...
connectedComponent(const arguments &args)
{
    ...
    ConnectedComponentsMetricsFilter->SetTol( args.tol );
    ...
    if(args.pthread > 0)
        ConnectedComponentsMetricsFilter->SetNumberOfThreads(args.pthread);
...
int main(int argc, char **argv)
{
    ...
    itk::ImageIOBase::Pointer imageIOref = itk::ImageIOFactory::CreateImageIO(refArg.getValue().c_str(),itk::ImageIOFactory::ReadMode);
    if( !imageIOref )
    {
    ...
    itk::ImageIOBase::Pointer imageIOtest = itk::ImageIOFactory::CreateImageIO(testArg.getValue().c_str(),itk::ImageIOFactory::ReadMode);
    if( !imageIOtest )
    {
        std::cerr << "Itk could not find suitable IO factory for the input" << std::endl;
        return EXIT_FAILURE;
