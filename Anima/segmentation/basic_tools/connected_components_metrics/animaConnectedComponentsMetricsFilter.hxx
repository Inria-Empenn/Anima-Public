...
::GenerateData()
{
    ...
    CCTestFilter->SetTol( m_Tol );
    CCTestFilter->SetDimension( m_Dimension );
    CCTestFilter->SetVerbose( false );
    if(this->GetNumberOfThreads() > 0)
        CCTestFilter->SetNumberOfThreads( this->GetNumberOfThreads() );
...
::PrintImagesOutput()
{
    ...
    while(!ccReferenceImageIt.IsAtEnd())
    {
        if((ccReferenceImageIt.Get()!=0) && (ccTestImageIt.Get()!=0))
        {
            outputVoxelIt.Set(1); // TP
        }
        if((ccReferenceImageIt.Get()==0) && (ccTestImageIt.Get()!=0))
        {
            outputVoxelIt.Set(2); // FP
        }
        if((ccReferenceImageIt.Get()!=0) && (ccTestImageIt.Get()==0))
        {
...
::PrintInfo()
{
    if(m_Verbose)
    {
        ...
        std::cout << "-- Volume difference: " << m_VolumeDifference << " mm3" << std::endl;
        std::cout << "-- Volume difference ratio: " << m_VolumeDifferenceRatio << std::endl;
        std::cout << std::endl;

        std::cout << "** DETECTION INFORMATION" << std::endl;

        std::cout << "* In Reference Image: " << std::endl;
        std::cout << "-- True positives rate: " << m_RefTruePositiveRate << std::endl;
        std::cout << "-- False negatives rate: " << m_FalseNegativeRate << std::endl;

        std::cout << "* In Test Image: " << std::endl;
        std::cout << "-- True positives rate: " << m_TestTruePositiveRate << std::endl;
        std::cout << "-- False positives rate: " << m_FalsePositiveRate << std::endl;
         std::cout << std::endl;

        std::cout << "** EVOLUTION INFORMATION" << std::endl;

        std::cout << "* In Reference Image: " << std::endl;
        std::cout << "-- Growing connected components rate: " << m_RefGrowRate << std::endl;
        std::cout << "-- Shrinking connected components rate: " << m_RefShrinkRate << std::endl;
        std::cout << "-- Same connected components rate: " << m_RefSameRate << std::endl;

        std::cout << "* In Test Image: " << std::endl;
        std::cout << "-- Growing connected components rate: " << m_TestGrowRate << std::endl;
        std::cout << "-- Shrinking connected components rate: " << m_TestShrinkRate << std::endl;
        std::cout << "-- Same connected components rate: " << m_TestSameRate << std::endl;
        std::cout << std::endl;
...
::PrintMetricsFile()
{
    if(m_metrics_text_filename.size()!=0)
    {
        ...
        else
        {
            ...
            fp << "-- Number of false negatives: " << m_nb_false_negative << std::endl;
            fp << " (connected components number: ";
            for(unsigned int k = 1; k < m_vect_true_positive_ref.size(); k++)
            {
                if(!m_vect_true_positive_ref[k])
            ...
            fp << "-- Number of false positives: " << m_nb_false_positive << std::endl;
            fp << " (connected components number: ";
            for(unsigned int k = 1; k < m_vect_true_positive_test.size(); k++)
            {
                if(!m_vect_true_positive_test[k])
