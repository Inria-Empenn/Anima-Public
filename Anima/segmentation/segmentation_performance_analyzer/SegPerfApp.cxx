...
bool CSegPerfApp::init(int argc, char *argv[])
{
    ...
    try
    {
        ...
        TCLAP::CmdLine cmd("Tools to analyze segmentation performances by comparison", ' ', "2.1");

        // Define a value argument and add it to the command line.
        TCLAP::ValueArg<std::string> oArgInputImg("i", "input", "Input image.", true, "", "string");

        cmd.add( oArgInputImg );

        // Define a value argument and add it to the command line.
        TCLAP::ValueArg<std::string> oArgRefImg("r", "ref", "Reference image to compare input image.", true, "", "string");

        cmd.add( oArgRefImg );

        // Define a value argument and add it to the command line.
        TCLAP::ValueArg<std::string> oArgBaseOutputName("o", "outputBase", "Base name for output files", false, "", "string");

        cmd.add( oArgBaseOutputName );

        // Define a value argument and add it to the command line.
        TCLAP::ValueArg<int> oArgNbThreads("t", "threads", "Number of threads", false, 0, "int");

        cmd.add( oArgNbThreads );

        // Define a value argument and add it to the command line.
        TCLAP::SwitchArg oArgAbout("A", "About", "Details on output metrics", false);

        cmd.add( oArgAbout );

        // Define a switch and add it to the command line.
        TCLAP::SwitchArg oArgSwitchText("T", "text", "Stores results into a text file.", false);

        cmd.add( oArgSwitchText );


        // Define a switch and add it to the command line.
        TCLAP::SwitchArg oArgSwitchXml("X", "xml", "Stores results into a xml file.", false);

        cmd.add( oArgSwitchXml );

        // Define a switch and add it to the command line.
        TCLAP::SwitchArg oArgSwitchScreen("S", "screen", "Print results on the screen when program ended.", false);

        cmd.add( oArgSwitchScreen );

        // Define a switch and add it to the command line.
        TCLAP::SwitchArg oArgSegEval("s", "SegmentationEvaluationMetrics", "Compute metrics to evaluate a segmentation.", false);

        cmd.add( oArgSegEval );

        // Define a switch and add it to the command line.
        TCLAP::SwitchArg oArgSwitchAdvancedEvaluation("a", "advancedEvaluation", "Compute results for each cluster (intra-lesion results)", false);

        cmd.add( oArgSwitchAdvancedEvaluation );

        // Define a switch and add it to the command line.
        TCLAP::SwitchArg oArgSwitchDetectionEval("l", "LesionDetectionMetrics", "Compute metrics to evaluate the detection of lesions along to a segmentation.", false);

        cmd.add( oArgSwitchDetectionEval );

        // Define a switch and add it to the command line.
        TCLAP::SwitchArg oArgSwitchSurfaceDist("d", "SurfaceEvaluation", "Surface distances evaluation.", false);

        cmd.add( oArgSwitchSurfaceDist );

        // Define a switch and add it to the command line.
        TCLAP::ValueArg<float> oArgSwitchDetectionLesionMinVolume("v", "MinLesionVolume", "Min volume of lesion for \"Lesions detection metrics\" in mm^3 (default 3mm^3).", false, 3.00, "float");

        cmd.add( oArgSwitchDetectionLesionMinVolume );

        // Define a switch and add it to the command line.
        TCLAP::ValueArg<float> oArgSwitchTPLMinOverlapRatio("x", "MinOverlapRatio", "Minimum overlap ratio to say if a lesion of the GT is detected. (default 0.10)", false, 0.10, "float");

        cmd.add( oArgSwitchTPLMinOverlapRatio );

        // Define a switch and add it to the command line.
        TCLAP::ValueArg<float> oArgSwitchTPLMaxFalsePositiveRatio("y", "MaxFalsePositiveRatio", "Maximum of false positive ratio to limit the detection of a lesion in GT if a lesion in the image is too big. (default 0.7)", false, 0.70, "float");

        cmd.add( oArgSwitchTPLMaxFalsePositiveRatio );

        // Define a switch and add it to the command line.
        TCLAP::ValueArg<float> oArgSwitchTPLMaxFalsePositiveRatioModerator("z", "MaxFalsePositiveRatioModerator", "Percentage of the regions overlapping the tested lesion is not too much outside of this lesion. (default 0.65)", false, 0.65, "float");
