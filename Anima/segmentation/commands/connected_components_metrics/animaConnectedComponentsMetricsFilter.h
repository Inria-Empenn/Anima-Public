...
public:
  ...
  using Self = ConnectedComponentsMetricsFilter;
  using Superclass = itk::ImageToImageFilter<ImageType, OutputType>;
  ...
  using OutputImagePointerType = typename OutputType::Pointer;
  using OutputPixelType = typename OutputType::PixelType;
  using OutputIteratorType = typename itk::ImageRegionIterator<OutputType>;

  using ConnectedComponentVolumeFilterType =
      anima::ConnectedComponentsVolumeFilter<ImageType>;
  ...
  void SetOutputFilenameCCRef(std::string outputFilename) {
    m_OutputCCReference_filename = outputFilename;
  }
  void SetOutputFilenameCCTest(std::string outputFilename) {
    m_OutputCCTest_filename = outputFilename;
  }

  void SetOutputFilenameDiffVoxelWise(std::string outputFilename) {
    m_OutputDiffVoxelWise_filename = outputFilename;
  }
  void SetOutputFilenameDiffTest(std::string outputFilename) {
    m_OutputDiffTest_filename = outputFilename;
  }
  void SetOutputFilenameDiffRef(std::string outputFilename) {
    m_OutputDiffRef_filename = outputFilename;
  }

  void SetOutputFilenameEvolutionTest(std::string outputFilename) {
    m_OutputEvolutionTest_filename = outputFilename;
  }
  void SetOutputFilenameEvolutionRef(std::string outputFilename) {
    m_OutputEvolutionRef_filename = outputFilename;
  }

  void SetOutputFilenameTextVolTest(std::string outputFilename) {
    m_volume_test_text_filename = outputFilename;
  }
  void SetOutputFilenameTextVolRef(std::string outputFilename) {
    m_volume_ref_text_filename = outputFilename;
  }
  void SetOutputFilenameTextMetrics(std::string outputFilename) {
    m_metrics_text_filename = outputFilename;
  }
  ...
  std::vector<unsigned short int> GetGrownTest() { return m_vect_grow_test; }
  std::vector<unsigned short int> GetShrinkTest() { return m_vect_shrink_test; }
  std::vector<unsigned short int> GetSameTest() { return m_vect_same_test; }

  std::vector<unsigned short int> GetGrownRef() { return m_vect_grow_ref; }
  std::vector<unsigned short int> GetShrinkRef() { return m_vect_shrink_ref; }
  std::vector<unsigned short int> GetSameRef() { return m_vect_same_ref; }

  std::vector<unsigned short int> GetTestTP() {
    return m_vect_true_positive_test;
  }
  std::vector<unsigned short int> GetRefTP() {
    return m_vect_true_positive_ref;
  }
...
private:
  ITK_DISALLOW_COPY_AND_ASSIGN(ConnectedComponentsMetricsFilter);
   ...
   * Images volumes
   * */
  unsigned int m_OriginalNumberOfObjectsTest; /*!< Original number of objects in
                                                 test image */
  unsigned int m_NumberOfObjectsTest;     /*!< Number of objects in test image
                                             (after removing small ones) */
  std::vector<double> m_vect_volume_test; /*!< Volume of each object */
  double m_TestTotalVolume;               /*!< Total volume of test image */

  unsigned int m_OriginalNumberOfObjectsRef; /*!< Original number of objects in
                                                reference image */
  unsigned int m_NumberOfObjectsRef; /*!< Number of objects in reference image
                                        (after removing small ones) */
  ...
  std::vector<unsigned short int>
      m_vect_true_positive_test; /*!< List of true positive objects in test
                                    image */
  std::vector<unsigned short int>
      m_vect_true_positive_ref; /*!< List of true positive objects in reference
                                   image */
  std::vector<unsigned short int>
      m_vect_false_positive_test; /*!< List of false positive objects in test
                                     image */

  unsigned int m_nb_true_positive_test; /*!< Number of true positive objects in
                                           test image */
  unsigned int m_nb_true_positive_ref;  /*!< Number of true positive objects in
                                           reference image */
  unsigned int m_nb_false_positive; /*!< Number of false positive objects (in
                                       test image) */
  unsigned int m_nb_false_negative; /*!< Number of false negative objects (in
                                       reference image) */
  ...
  unsigned int m_nb_grow_ref;   /*!< Number of objects that will grow (in
                                   reference image) */
  unsigned int m_nb_shrink_ref; /*!< Number of objects that will shrink (in
                                   reference image) */
  unsigned int m_nb_same_ref;   /*!< Number of objects that stay the same (in
                                   reference image) */

  unsigned int
      m_nb_grow_test; /*!< Number of objects have grown (in test image) */
  unsigned int
      m_nb_shrink_test; /*!< Number of objects have shrinked (in test image) */
  unsigned int m_nb_same_test; /*!< Number of objects that stay the same (in
                                  test image) */

  std::vector<unsigned short int>
      m_vect_grow_test; /*!< List of objects that will grow (in reference image)
                         */
  std::vector<unsigned short int>
      m_vect_shrink_test; /*!< List of objects that will shrink (in reference
                             image) */
  std::vector<unsigned short int>
      m_vect_same_test; /*!< List of objects that stay the same (in reference
                           image) */

  std::vector<unsigned short int>
      m_vect_grow_ref; /*!< List of objects have grown (in test image) */
  std::vector<unsigned short int>
      m_vect_shrink_ref; /*!< List of objects have shrinked (in test image) */
  std::vector<unsigned short int>
      m_vect_same_ref; /*!< List of objects that stay the same (in test image)
                        */
  ...
  double m_VolumeDifference; /*!< Volume Difference –  absolute difference in
                                volumes */
  double m_VolumeDifferenceRatio; /*!< Volume Difference Ratio –  absolute
                                     difference in volumes divided by the true
                                     volume */

  double m_FalsePositiveRate;   /*!< Object wise false positive rate */
  double m_FalseNegativeRate;   /*!< Object wise false negative rate */
  double m_RefTruePositiveRate; /*!< Object wise true positive rate in reference
                                   image (= recall) */
  double m_TestTruePositiveRate; /*!< Object wise true positive rate in test
                                    image (= precision) */
