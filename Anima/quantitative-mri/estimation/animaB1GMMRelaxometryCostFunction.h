 ...
 * following a gaussian mixture EPG decay model. The cost function includes (via
 * variable projection) estimation of compartment weights
 *
 */
...
public:
  ...
  using Self = B1GMMRelaxometryCostFunction;
  ...
  void SetGaussianMeans(std::vector<double> &val) {
    m_GaussianMeans = val;
    m_TruncatedGaussianIntegrals.clear();
  }
  void SetGaussianVariances(std::vector<double> &val) {
    m_GaussianVariances = val;
    m_TruncatedGaussianIntegrals.clear();
  }
