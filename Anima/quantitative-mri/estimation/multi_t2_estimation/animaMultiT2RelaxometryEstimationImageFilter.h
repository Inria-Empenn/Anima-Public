...
/**
 * @brief Implements multi-peak T2 relaxometry estimation (with or without
 * regularization)
 *
 * Multi-peak estimation is performed as discussed in several papers:
 * Layton et al. Modelling and estimation of multicomponent T2 distributions.
 * IEEE TMI, 32(8):1423-1434. 2013. Prasloski et al. Applications of stimulated
 * echo correction to multicomponent T2 analysis. MRM, 67(6):1803-1814. 2012.
 * Yoo et al. Non-local spatial regularization of MRI T2 relaxa- tion images for
 * myelin water quantification. MICCAI, pp 614-621. 2013.
 */
...
public:
  ...
  using Self = MultiT2RelaxometryEstimationImageFilter;
  ...
  itkTypeMacro(MultiT2RelaxometryEstimationImageFilter,
  ...
  using PatchSearcherType =
      anima::NonLocalT2DistributionPatchSearcher<VectorOutputImageType,
                                                 InputImageType>;
  using RegularizationCostFunctionType =
      anima::MultiT2RegularizationCostFunction;
  using RegularizationCostFunctionPointer =
      RegularizationCostFunctionType::Pointer;
  ...
  std::vector<double> &GetT2CompartmentValues() {
    return m_T2CompartmentValues;
  }

protected:
  MultiT2RelaxometryEstimationImageFilter() : Superclass() {
  ...
  void ComputeTikhonovPrior(const IndexType &refIndex,
                            OutputVectorType &refDistribution,
                            PatchSearcherType &nlPatchSearcher,
                            itk::OptimizerParameters<double> &priorDistribution,
                            std::vector<double> &workDataWeights,
                            std::vector<OutputVectorType> &workDataSamples);

  double ComputeRegularizedSolution(
      RegularizationCostFunctionType *regularizationCost, double &lambdaSq,
      itk::OptimizerParameters<double> &t2OptimizedWeights, double &m0Value);

  //! Implements Canales et al. L-curve regularization weight optimization (see
  //! Media 2021)
  double ComputeLCurveRegularizedSolution(
      RegularizationCostFunctionType *regularizationCost,
      itk::OptimizerParameters<double> &t2OptimizedWeights, double &m0Value);
