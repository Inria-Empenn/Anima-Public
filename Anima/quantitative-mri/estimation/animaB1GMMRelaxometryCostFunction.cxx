...
void B1GMMRelaxometryCostFunction::PrepareDataForLLS() const {
  ...
  // Contains int_{space of ith distribution} EPG(t2, b1, jth echo) G(t2, mu_i,
  // sigma_i) d t2
  ...
  if (m_TruncatedGaussianIntegrals.size() != numDistributions) {
    ...
    for (unsigned int i = 0; i < numDistributions; ++i)
      m_TruncatedGaussianIntegrals[i] =
          0.5 *
          (1.0 + boost::math::erf(m_GaussianMeans[i] /
                                  std::sqrt(2.0 * m_GaussianVariances[i])));
  }
  ...
  for (unsigned int i = 0; i < numDistributions; ++i) {
    ...
    if (m_UniformPulses) {
      ...
      double minInterestZoneValue =
          m_GaussianMeans[i] - 5.0 * std::sqrt(m_GaussianVariances[i]);
      double maxInterestZoneValue =
          m_GaussianMeans[i] + 5.0 * std::sqrt(m_GaussianVariances[i]);
    ...
    for (unsigned int j = 0; j < numT2Signals; ++j) {
      m_PredictedSignalAttenuations(j, i) =
          predictedSignals[j] / m_TruncatedGaussianIntegrals[i];
  ...
  for (unsigned int i = 0; i < numDistributions; ++i) {
    ...
    for (unsigned int j = 0; j < numT2Signals; ++j)
      m_FSignals[i] +=
          m_PredictedSignalAttenuations(j, i) * m_T2RelaxometrySignals[j];

    for (unsigned int j = i; j < numDistributions; ++j) {
      m_CholeskyMatrix(i, j) = 0;
      for (unsigned int k = 0; k < numT2Signals; ++k)
        m_CholeskyMatrix(i, j) += m_PredictedSignalAttenuations(k, i) *
                                  m_PredictedSignalAttenuations(k, j);
...
void B1GMMRelaxometryCostFunction::SolveLinearLeastSquares() const {
  unsigned int numT2Signals = m_T2RelaxometrySignals.size();
  unsigned int numDistributions = m_GaussianMeans.size();
  ...
  for (unsigned int i = 0; i < numT2Signals; ++i) {
    ...
    for (unsigned int j = 0; j < numDistributions; ++j)
      m_Residuals[i] -=
          m_PredictedSignalAttenuations(i, j) * m_OptimalT2Weights[j];
