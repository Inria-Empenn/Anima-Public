...
public:
  ...
  using Self = CombinedRelaxometryEstimationImageFilter;
  ...
  itkTypeMacro(CombinedRelaxometryEstimationImageFilter,
  ...
  using ThreadStruct = typename Superclass::ThreadStruct;
  ...
  void SetT1FlipAngles(double singleAngle, unsigned int numAngles) {
    m_T1FlipAngles = std::vector<double>(numAngles, singleAngle);
  }
...
protected:
  CombinedRelaxometryEstimationImageFilter() : Superclass() {
