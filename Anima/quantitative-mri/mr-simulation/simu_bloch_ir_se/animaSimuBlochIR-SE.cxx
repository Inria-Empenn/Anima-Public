...
{
    TCLAP::CmdLine cmd("SimuBlochIR-SE: Simulator for Inversion Recovery - Spin Echo\nINRIA / IRISA - VisAGeS/Empenn Team", ' ',ANIMA_VERSION);
  ...
  // Create the filter: estimate the sequence of Inversion Recovery
  ...
  filter->SetTR(TR);
  filter->SetTE(TE);
  filter->SetTI(TI);//changed for IR-SE
  filter->Update();
