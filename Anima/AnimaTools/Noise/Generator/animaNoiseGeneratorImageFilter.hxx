...
namespace anima
{
    ...
    void
    NoiseGeneratorImageFilter<ImageType>::GenerateOutputInformation()
    {
        //-------------------------------
    ...
    template <class ImageType>
    void
    NoiseGeneratorImageFilter<ImageType>::BeforeThreadedGenerateData()
    {
    ...
    template <class ImageType>
    void
    NoiseGeneratorImageFilter<ImageType>::DynamicThreadedGenerateData(const OutputImageRegionType &outputRegionForThread)
    {
        ...
        std::normal_distribution<double> normDistr(0.0, m_NoiseSigma);
        ...
        while (!inputIterator.IsAtEnd())
        {
            ...
            for (unsigned int i = 0; i < m_NumberOfReplicates; ++i)
            {
                double realNoise = normDistr(m_Generators[threadId]);
                double data = refData + realNoise;

                if (!m_UseGaussianDistribution)
                {
                    double imagNoise = normDistr(m_Generators[threadId]);
