...
TensorCorrelationImageToImageMetric<
    ...
    ImageDimension>::TensorCorrelationImageToImageMetric() {
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
typename TensorCorrelationImageToImageMetric<
...
TensorCorrelationImageToImageMetric<
  ...
  unsigned int tensorDimension =
      floor((std::sqrt((double)(8 * vectorSize + 1)) - 1) / 2.0);
  ...
  for (unsigned int i = 0; i < this->m_NumberOfPixelsCounted; ++i) {
    ...
    if (this->m_Interpolator->IsInsideBuffer(transformedIndex)) {
      ...
      if (this->GetModelRotation() != Superclass::NONE) {
        // Rotating tensor
        anima::GetTensorFromVectorRepresentation(movingValue, tmpMat,
        ...
        anima::GetVectorRepresentation(currentTensor, movingValue, vectorSize,
                                       true);
      }
      ...
      for (unsigned int j = 0; j < tensorDimension; ++j)
        for (unsigned int k = 0; k <= j; ++k) {
          ...
          mRS +=
              m_FixedImageValues[i][pos_internal] * movingValue[pos_internal];
  ...
  movingDenominator = mSS + mST * mST *
                                (3.0 * this->m_NumberOfPixelsCounted *
                                     m_LogEpsilon * m_LogEpsilon -
                                 2.0);
  ...
  double measure = measureNumerator * measureNumerator /
                   (m_FixedDenominator * movingDenominator);
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
void TensorCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
  ...
  unsigned int tensorDimension =
      floor((sqrt((double)(8 * vectorSize + 1)) - 1) / 2.0);
  ...
  while (!ti.IsAtEnd()) {
    ...
    for (unsigned int i = 0; i < tensorDimension; ++i)
      for (unsigned int j = 0; j <= i; ++j) {
        ...
        m_FixedDenominator +=
            fixedValue[pos_internal] * fixedValue[pos_internal];
  ...
  m_FixedDenominator +=
      m_FixedTProduct * m_FixedTProduct *
      (3.0 * this->m_NumberOfPixelsCounted * m_LogEpsilon * m_LogEpsilon - 2.0);
}
