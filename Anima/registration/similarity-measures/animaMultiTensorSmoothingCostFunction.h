...
class ANIMAMCMSIMILARITY_EXPORT MultiTensorSmoothingCostFunction
    : public itk::SingleValuedCostFunction {
public:
  /** Standard class typedefs. */
  using Self = MultiTensorSmoothingCostFunction;
  ...
  using TensorType = anima::BaseCompartment::Matrix3DType;
...
protected:
  ...
  void ComputeMatrixTraces(const TensorType &matrix, double &matrixTrace,
                           double &squaredMatrixTrace) const;
...
private:
  ...
  mutable std::vector<std::vector<double>> m_ReferenceReferenceDeterminants,
      m_ReferenceMovingDeterminants, m_MovingMovingDeterminants;
  mutable std::vector<std::vector<double>> m_ReferenceReferenceTraces,
      m_ReferenceMovingTraces;
  mutable std::vector<std::vector<double>> m_ReferenceReferenceTraceDifferences,
      m_ReferenceMovingTraceDifferences;
