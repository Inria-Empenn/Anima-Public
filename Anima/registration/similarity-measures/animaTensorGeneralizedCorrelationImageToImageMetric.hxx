...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
typename TensorGeneralizedCorrelationImageToImageMetric<
...
TensorGeneralizedCorrelationImageToImageMetric<
  ...
  for (unsigned int i = 0; i < this->m_NumberOfPixelsCounted; ++i) {
    ...
    bool isInside = movingImage->TransformPhysicalPointToContinuousIndex(
    ...
    if (this->m_Interpolator->IsInsideBuffer(transformedIndex)) {
      ...
      if (this->GetModelRotation() != Superclass::NONE) {
        // Rotating tensor
        anima::GetTensorFromVectorRepresentation(movingValues[i], tmpMat,
                                                 tensorDimension, true);

        if (this->GetModelRotation() == Superclass::FINITE_STRAIN)
          anima::RotateSymmetricMatrix(tmpMat, this->m_OrientationMatrix,
                                       currentTensor);
        else {
          ...
          eigenComputer.ComputeEigenValuesAndVectors(tmpMatItk, eigVals,
                                                     eigVecs);
          anima::ExtractPPDRotationFromJacobianMatrix(
              this->m_OrientationMatrix, ppdOrientationMatrix, eigVecs);
          anima::RotateSymmetricMatrix(tmpMat, ppdOrientationMatrix,
                                       currentTensor);
        }

        anima::GetVectorRepresentation(currentTensor, movingValues[i],
                                       vectorSize, true);
      }
  ...
  for (unsigned int i = 0; i < this->m_NumberOfPixelsCounted; ++i) {
    for (unsigned int j = 0; j < vectorSize; ++j) {
      for (unsigned int k = 0; k < vectorSize; ++k)
        Sigma_XY(j, k) += (m_FixedImageValues[i][j] - m_FixedMean[j]) *
                          (movingValues[i][k] - movingMean[k]);

      for (unsigned int k = j; k < vectorSize; ++k)
        Sigma_YY(j, k) += (movingValues[i][j] - movingMean[j]) *
                          (movingValues[i][k] - movingMean[k]);
    }
  }
  ...
  if (m_OrientationPenalty) {
    vnl_matrix<double> tmpXEVecs(tensorDimension, tensorDimension),
        tmpYEVecs(tensorDimension, tensorDimension);
    ...
    itk::SymmetricEigenAnalysis<vnl_matrix<double>, vnl_diag_matrix<double>,
                                vnl_matrix<double>>
        eigenComputer(tensorDimension);

    anima::GetTensorFromVectorRepresentation(m_FixedMean, tmpMat,
                                             tensorDimension, true);
    eigenComputer.ComputeEigenValuesAndVectors(tmpMat, tmpEigX, tmpXEVecs);

    anima::GetTensorFromVectorRepresentation(movingMean, tmpMat,
                                             tensorDimension, true);
...
template <class TFixedImagePixelType, class TMovingImagePixelType,
          unsigned int ImageDimension>
void TensorGeneralizedCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
  ...
  for (unsigned int k = 0; k < this->m_NumberOfPixelsCounted; ++k) {
    for (unsigned int i = 0; i < vectorSize; ++i) {
      for (unsigned int j = i; j < vectorSize; ++j)
        covarianceMatrix(i, j) += (m_FixedImageValues[k][i] - m_FixedMean[i]) *
                                  (m_FixedImageValues[k][j] - m_FixedMean[j]);
    }
  }
  ...
  m_leCalculator->GetTensorPower(covarianceMatrix,
                                 m_FixedHalfInvCovarianceMatrix, -0.5);
}
...
void TensorGeneralizedCorrelationImageToImageMetric<
    TFixedImagePixelType, TMovingImagePixelType,
    ImageDimension>::PrintSelf(std::ostream &os, itk::Indent indent) const {
