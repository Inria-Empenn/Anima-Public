...
template <unsigned int ImageDimension>
void PyramidalBlockMatchingBridge<ImageDimension>::SetInitialTransform(
    std::string initialTransformFile) {
  if (initialTransformFile != "") {
    itk::TransformFileReader::Pointer tmpTrRead =
    ...
    try {
      ...
      itk::TransformFileReader::TransformListType trsfList =
          *(tmpTrRead->GetTransformList());
      itk::TransformFileReader::TransformListType::iterator tr_it =
          trsfList.begin();

      m_InitialTransform =
          dynamic_cast<AffineTransformType *>((*tr_it).GetPointer());

    } catch (itk::ExceptionObject &e) {
      std::cerr << "Unable to read initial transform... Exiting..."
                << std::endl;
    }
  }
}
...
template <unsigned int ImageDimension>
void PyramidalBlockMatchingBridge<ImageDimension>::SetDirectionTransform(
    std::string directionTransformFile) {
  if (directionTransformFile != "") {
    ...
    try {
      ...
      m_DirectionTransform =
          dynamic_cast<AffineTransformType *>((*tr_it).GetPointer());

    } catch (itk::ExceptionObject &e) {
      std::cerr << "Unable to read direction transform... Exiting..."
                << std::endl;
    }
  }
}
...
void PyramidalBlockMatchingBridge<ImageDimension>::Update() {
  ...
  // progress management
  m_progressReporter = new itk::ProgressReporter(
  ...
  bool invertInputs =
      (m_RegistrationPointLocation < 0.5) && (m_SymmetryType == Kissing);
  if (invertInputs) {
  ...
  for (unsigned int i = 0; i < GetNumberOfPyramidLevels() && !m_Abort; ++i) {
    ...
    typename InputImageType::Pointer refImage =
        m_ReferencePyramid->GetOutput(i);
    ...
    mainMatcher->SetBlockVarianceThreshold(GetStDevThreshold() *
                                           GetStDevThreshold());
    ...
    if (m_Verbose) {
      ...
      std::cout << "Image size: "
                << refImage->GetLargestPossibleRegion().GetSize() << std::endl;
    }

    // Init bm registration method
    switch (m_SymmetryType) {
    case Asymmetric: {
      typedef typename anima::AsymmetricBMRegistrationMethod<InputImageType>
          BlockMatchRegistrationType;
      m_bmreg = BlockMatchRegistrationType::New();
      break;
    }

    case Symmetric: {
      typedef typename anima::SymmetricBMRegistrationMethod<InputImageType>
          BlockMatchRegistrationType;
      typename BlockMatchRegistrationType::Pointer tmpReg =
          BlockMatchRegistrationType::New();
      ...
      reverseMatcher->SetBlockVarianceThreshold(GetStDevThreshold() *
                                                GetStDevThreshold());
    ...
    default: {
      typedef
          typename anima::KissingSymmetricBMRegistrationMethod<InputImageType>
              BlockMatchRegistrationType;
      typename BlockMatchRegistrationType::Pointer tmpReg =
          BlockMatchRegistrationType::New();
    ...
    if (m_progressCallback) {
      ...
      // we need to create a new progressReporter with more iterations
      // (m_progressReporter), to listen to progress events from bmreg and to
      // send new ones to m_progressCallback
    ...
    typedef anima::ResampleImageFilter<InputImageType, InputImageType,
                                       typename AgregatorType::ScalarType>
        ResampleFilterType;

    typename ResampleFilterType::Pointer refResampler =
    ...
    switch (GetTransform()) {
    case Translation:
      mainMatcher->SetBlockTransformType(
          BlockMatcherType::Superclass::Translation);
      if (reverseMatcher)
        reverseMatcher->SetBlockTransformType(
            BlockMatcherType::Superclass::Translation);
      break;
    case Rigid:
      ...
      if (reverseMatcher)
        reverseMatcher->SetBlockTransformType(
            BlockMatcherType::Superclass::Rigid);
      break;
    case Directional_Affine:
      mainMatcher->SetBlockTransformType(
          BlockMatcherType::Superclass::Directional_Affine);
    ...
    default:
      ...
      if (reverseMatcher)
        reverseMatcher->SetBlockTransformType(
            BlockMatcherType::Superclass::Affine);
      break;
    }
    ...
    if (reverseMatcher) {
      ...
      reverseMatcher->SetOptimizerMaximumIterations(
    ...
    try {
      ...
      std::cerr << "ExceptionObject caught in bmreg startregistration ! " << err
                << std::endl;
      exit(-1);
    }

    if ((GetOutputTransformType() == outAnisotropic_Sim) ||
        (GetOutputTransformType() == outAffine))
      m_EstimationBarycenter = agreg->GetEstimationBarycenter();

    // Polyrigid will have to be handled here
    AffineTransformType *tmpTrsf =
        dynamic_cast<AffineTransformType *>(m_OutputTransform.GetPointer());
  ...
  typedef
      typename anima::ResampleImageFilter<InputImageType, InputImageType,
                                          typename AgregatorType::ScalarType>
          ResampleFilterType;
...
template <unsigned int ImageDimension>
void PyramidalBlockMatchingBridge<ImageDimension>::ManageProgress(
    itk::Object *caller, const itk::EventObject &event, void *clientData) {
  PyramidalBlockMatchingBridge *source =
      reinterpret_cast<PyramidalBlockMatchingBridge *>(clientData);
...
void PyramidalBlockMatchingBridge<ImageDimension>::WriteOutputs() {
  ...
  if (GetOutputTransformFile() != "") {
    std::cout << "Writing output transform to: " << GetOutputTransformFile()
              << std::endl;
  ...
  typename AffineTransformType::OffsetType transformOffset =
      tmpTrsf->GetOffset();
  vnl_svd<typename AffineTransformType::MatrixType::ValueType>
      UWVLinearMatrixSVD(linearMatrix.GetVnlMatrix().as_matrix());
  vnl_matrix<typename AffineTransformType::MatrixType::ValueType> leftRot =
      UWVLinearMatrixSVD.U() * vnl_determinant(UWVLinearMatrixSVD.U());
  vnl_matrix<typename AffineTransformType::MatrixType::ValueType> rightRot =
      UWVLinearMatrixSVD.V() * vnl_determinant(UWVLinearMatrixSVD.V());
  vnl_diag_matrix<typename AffineTransformType::MatrixType::ValueType> scal =
      UWVLinearMatrixSVD.W();
  ...
  for (unsigned int i = 0; i < ImageDimension; ++i) {
    ...
    for (unsigned int j = 0; j < ImageDimension; ++j) {
      ...
      similarityOffset[i] -=
          linearPartSimilarity(i, j) * m_EstimationBarycenter[j];
    }
  }
  ...
  if (m_outputNearestRigidTransformFile != "") {
    ...
    itk::TransformFileWriter::Pointer rigidWriter =
        itk::TransformFileWriter::New();
    rigidWriter->SetInput(rigidTransform);
    rigidWriter->SetFileName(GetOutputNearestRigidTransformFile());
    std::cout << "Writing output nearest rigid transform to: "
              << GetOutputNearestRigidTransformFile() << std::endl;
    rigidWriter->Update();
  }
  ...
  if (m_outputNearestSimilarityTransformFile != "") {
    ...
    itk::TransformFileWriter::Pointer similarityWriter =
        itk::TransformFileWriter::New();
    similarityWriter->SetInput(similarityTransform);
    similarityWriter->SetFileName(GetOutputNearestSimilarityTransformFile());
    std::cout << "Writing output nearest similarity transform to: "
              << GetOutputNearestSimilarityTransformFile() << std::endl;
...
void PyramidalBlockMatchingBridge<ImageDimension>::SetupPyramids() {
  ...
  InputImagePointer initialFloatingImage =
      const_cast<InputImageType *>(m_FloatingImage.GetPointer());

  // Compute initial transform if needed to get a decent initial Floating image
  if (m_InitialTransform.IsNotNull()) {
    typename ResampleFilterType::Pointer tmpResample =
        ResampleFilterType::New();
    tmpResample->SetTransform(m_InitialTransform);
    tmpResample->SetInput(m_FloatingImage);

    tmpResample->SetSize(
        m_ReferenceImage->GetLargestPossibleRegion().GetSize());
    tmpResample->SetOutputOrigin(m_ReferenceImage->GetOrigin());
    tmpResample->SetOutputSpacing(m_ReferenceImage->GetSpacing());
    tmpResample->SetOutputDirection(m_ReferenceImage->GetDirection());
    tmpResample->SetDefaultPixelValue(m_FloatingMinimalValue);
    ...
    if (m_TransformInitializationType != Identity) {
      ...
      typename ImageCalculatorType::Pointer fixedCalculator =
          ImageCalculatorType::New();
      fixedCalculator->SetImage(offsetRefImage);
      fixedCalculator->Compute();
      typename ImageCalculatorType::VectorType fixedBar =
      ...
      typename ImageCalculatorType::Pointer movingCalculator =
          ImageCalculatorType::New();
      movingCalculator->SetImage(offsetFloatingImage);
      movingCalculator->Compute();
      typename ImageCalculatorType::VectorType movingBar =
          movingCalculator->GetCenterOfGravity();

      m_InitialTransform->SetOffset(movingBar - fixedBar);

      if ((m_TransformInitializationType == ClosestTransform) &&
          (m_OutputTransformType != outTranslation)) {
        typename ImageCalculatorType::VectorType fixedPrincipalMom =
            fixedCalculator->GetPrincipalMoments();
        typename ImageCalculatorType::ScalarType fixedMass =
            fixedCalculator->GetTotalMass();

        typename ImageCalculatorType::VectorType movingPrincipalMom =
            movingCalculator->GetPrincipalMoments();
        typename ImageCalculatorType::ScalarType movingMass =
            movingCalculator->GetTotalMass();
        ...
        if ((m_OutputTransformType == outAnisotropic_Sim) ||
            (m_OutputTransformType == outAffine))
    ...
    using MinMaxImageFilterType =
        itk::MinimumMaximumImageFilter<InputImageType>;
    ...
    if ((minMaxFilter->GetMinimum() == minMaxFilter->GetMaximum()) &&
        (m_TransformInitializationType == Identity)) {
      std::cout << "Identity initialization outputs an empty image, "
                   "initializing with centers of mass"
                << std::endl;
  ...
  if (m_BlockGenerationMask) {
    typedef anima::ResampleImageFilter<MaskImageType, MaskImageType,
                                       typename AgregatorType::ScalarType>
        MaskResampleFilterType;

    typename MaskResampleFilterType::Pointer maskResampler =
