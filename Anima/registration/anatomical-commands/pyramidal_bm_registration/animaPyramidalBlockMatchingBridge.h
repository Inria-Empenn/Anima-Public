...
public:
  ...
  using AgregatorType = anima::BaseTransformAgregator<ImageDimension>;
  using BaseTransformType = typename AgregatorType::BaseOutputTransformType;
  using BaseTransformPointer = typename BaseTransformType::Pointer;

  using AffineTransformType =
      itk::AffineTransform<typename AgregatorType::ScalarType, ImageDimension>;
  ...
  using Self = PyramidalBlockMatchingBridge;
  ...
  enum InitializationType { Identity = 0, GravityCenters, ClosestTransform };
  ...
  enum Agregator { MEstimation = 0, LeastSquares, LeastTrimmedSquares };
  ...
  std::string GetOutputNearestRigidTransformFile() {
    return m_outputNearestRigidTransformFile;
  }
  void SetOutputNearestRigidTransformFile(std::string outputNRTransformFile) {
    m_outputNearestRigidTransformFile = outputNRTransformFile;
  }

  std::string GetOutputNearestSimilarityTransformFile() {
    return m_outputNearestSimilarityTransformFile;
  }
  void SetOutputNearestSimilarityTransformFile(
      std::string outputNearestSimilarityTransformFile) {
    m_outputNearestSimilarityTransformFile =
        outputNearestSimilarityTransformFile;
  }
  ...
  InitializationType GetTransformInitializationType() {
    return m_TransformInitializationType;
  }
  void SetTransformInitializationType(
      InitializationType transformInitializationType) {
    m_TransformInitializationType = transformInitializationType;
  }
