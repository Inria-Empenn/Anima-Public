...
class BlockMatchingInitializer : public itk::Object {
public:
  using Superclass = itk::Object;
  using Self = BlockMatchingInitializer<PixelType, NDimensions>;
  using Pointer = itk::SmartPointer<Self>;
  using ConstPointer = itk::SmartPointer<const Self>;

  using ScalarImageType = itk::Image<PixelType, NDimensions>;
  using WeightImageType = itk::Image<double, NDimensions>;
  using WeightImagePointer = typename WeightImageType::Pointer;
  using IndexType = typename ScalarImageType::IndexType;
  using PointType = typename ScalarImageType::PointType;
  using VectorImageType = itk::VectorImage<PixelType, NDimensions>;
  using MaskImageType = itk::Image<unsigned char, NDimensions>;
  ...
  using ImageRegionType = typename ScalarImageType::RegionType;
  ...
  ScalarImageType *GetReferenceScalarImage(unsigned int i) {
    return m_ReferenceScalarImages[i];
  }
  VectorImageType *GetReferenceVectorImage(unsigned int i) {
    return m_ReferenceVectorImages[i];
  }
  ...
  void RegionBlockGenerator(BlockGeneratorThreadStruct *workStr,
...
protected:
  ...
  virtual void InitializeThreading(unsigned int maskIndex,
  ...
  bool CheckScalarVariance(ScalarImageType *refImage, ImageRegionType &region,
                           double &blockVariance);
  virtual bool CheckOrientedModelVariance(unsigned int imageIndex,
                                          ImageRegionType &region,
                                          double &blockVariance,
                                          BlockGeneratorThreadStruct *workStr,
                                          unsigned int threadId);

  bool ProgressCounter(std::vector<unsigned int> &counter,
                       std::vector<unsigned int> &bounds);

  struct pair_comparator {
    bool operator()(
        const std::pair<double, std::pair<PointType, ImageRegionType>> &f,
