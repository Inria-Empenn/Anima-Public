...
public:
  ...
  using Self = OrientedModelBaseResampleImageFilter;
  using InputImageType = TImageType;
  using TOutputImage = TImageType;
  itkStaticConstMacro(ImageDimension, unsigned int,
                      InputImageType::ImageDimension);

  using GeometryImageType =
      itk::Image<unsigned char, itkGetStaticConstMacro(ImageDimension)>;
  using GeometryImagePointer = typename GeometryImageType::Pointer;

  using Superclass = anima::MaskedImageToImageFilter<TImageType, TImageType>;
  using Pointer = itk::SmartPointer<Self>;
  using ConstPointer = itk::SmartPointer<const Self>;

  using InterpolatorType =
      itk::InterpolateImageFunction<InputImageType, TInterpolatorPrecisionType>;
  ...
  itkTypeMacro(OrientedModelBaseResampleImageFilter,
  ...
  using TransformType = itk::Transform<TInterpolatorPrecisionType,
...
protected:
  ...
  template <class T> inline bool isZero(itk::VariableLengthVector<T> &dataVec) {
  ...
  //! Utility function to initialize output images pixel to zero for vector
  //! images
  template <class T>
  inline void InitializeZeroPixel(itk::VariableLengthVector<T> &zeroPixel) {
    zeroPixel.Fill(0.0);
  }

  //! Utility function to initialize output images pixel to zero for all images
  //! except vector images
  ...
  /**
   * Compute local re-orientation transformation matrix from non linear
   * transform (rotation if finite strain, Jacobian otherwise)
   */
  void ComputeLocalJacobianMatrix(InputIndexType &index,
  ...
  /**
   * For some models / re-orientation schemes, the transform induced Jacobiam
   * matrix needs to be used to get rotation parameters if needed, this method
   * should be overloaded
   */
  virtual void ComputeRotationParametersFromReorientationMatrix(
      vnl_matrix<double> &reorientationMatrix,
      vnl_matrix<double> &modelOrientationMatrix);

  //! Needs to be implemented in sub-classes, does the actual re-orientation of
  //! the model
  virtual void
