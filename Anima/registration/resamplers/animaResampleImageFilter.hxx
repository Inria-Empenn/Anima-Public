...
ResampleImageFilter<TInputImage, TOutputImage,
                    TInterpolatorPrecisionType>::ResampleImageFilter() {
  ...
  m_Transform =
      itk::IdentityTransform<TInterpolatorPrecisionType, ImageDimension>::New();
...
template <class TInputImage, class TOutputImage,
          class TInterpolatorPrecisionType>
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::PrintSelf(std::ostream &os,
  ...
  os << indent << "UseReferenceImage: " << (m_UseReferenceImage ? "On" : "Off")
     << std::endl;
  os << indent << "Scale intensities with Jacobian : "
     << (m_ScaleIntensitiesWithJacobian ? "On" : "Off") << std::endl;
  return;
}

/**
 * Returns a class copy with important specific variables set, not the greatest
 * implementation especially in terms of interpolators and other internal
 * pointers copy (not done here). Use with extreme caution
 */
template <class TInputImage, class TOutputImage,
          class TInterpolatorPrecisionType>
itk::LightObject::Pointer
ResampleImageFilter<TInputImage, TOutputImage,
                    TInterpolatorPrecisionType>::InternalClone() const {
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::SetOutputSpacing(const double *spacing) {
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::SetOutputOrigin(const double *origin) {
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::SetTransform(TransformType *transform) {
  if (m_Transform != transform) {
    m_Transform = transform;

    const MatrixTransformType *tmpTrsf =
        dynamic_cast<const MatrixTransformType *>(transform);
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::BeforeThreadedGenerateData() {
  if (!m_Transform) {
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::AfterThreadedGenerateData() {
...
template <class TInputImage, class TOutputImage,
          class TInterpolatorPrecisionType>
void ResampleImageFilter<TInputImage, TOutputImage,
  ...
  typedef itk::ContinuousIndex<TInterpolatorPrecisionType, ImageDimension>
      ContinuousIndexType;
  ...
  while (!outIt.IsAtEnd()) {
    ...
    bool isInside = inputPtr->TransformPhysicalPointToContinuousIndex(
        inputPoint, inputIndex);

    // Evaluate input at right position and copy to the output
    if (isInside && m_Interpolator->IsInsideBuffer(inputIndex)) {
      PixelType pixval;
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::GenerateInputRequestedRegion() {
...
template <class TInputImage, class TOutputImage,
          class TInterpolatorPrecisionType>
const typename ResampleImageFilter<
    TInputImage, TOutputImage, TInterpolatorPrecisionType>::OutputImageType *
ResampleImageFilter<TInputImage, TOutputImage,
                    TInterpolatorPrecisionType>::GetReferenceImage() const {
  Self *surrogate = const_cast<Self *>(this);
  const OutputImageType *referenceImage = static_cast<const OutputImageType *>(
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::SetReferenceImage(const TOutputImage *image) {
...
void ResampleImageFilter<
    TInputImage, TOutputImage,
    TInterpolatorPrecisionType>::GenerateOutputInformation() {
...
ResampleImageFilter<TInputImage, TOutputImage,
                    TInterpolatorPrecisionType>::GetMTime() const {
...
double
ResampleImageFilter<TInputImage, TOutputImage,
                    TInterpolatorPrecisionType>::ComputeLinearJacobianValue() {
  vnl_matrix_fixed<double, ImageDimension, ImageDimension> jacMatrix;
  const MatrixTransformType *matrixTrsf =
      dynamic_cast<const MatrixTransformType *>(m_Transform.GetPointer());
...
ResampleImageFilter<TInputImage, TOutputImage, TInterpolatorPrecisionType>::
  ...
  endIndDef =
      startIndDef + this->GetOutput()->GetLargestPossibleRegion().GetSize();
  ...
  vnl_matrix_inverse<double> invDeltaMatrix(deltaMatrix.as_matrix());
  deltaMatrix = invDeltaMatrix.inverse();
