...
void OrientedModelBaseResampleImageFilter<
    TImageType, TInterpolatorPrecisionType>::InitializeInterpolator() {
  typedef anima::VectorModelLinearInterpolateImageFunction<
...
template <typename TImageType, typename TInterpolatorPrecisionType>
void OrientedModelBaseResampleImageFilter<
    TImageType, TInterpolatorPrecisionType>::GenerateInputRequestedRegion() {
...
template <typename TImageType, typename TInterpolatorPrecisionType>
void OrientedModelBaseResampleImageFilter<
    TImageType, TInterpolatorPrecisionType>::GenerateOutputInformation() {
  ...
  this->GetOutput()->SetNumberOfComponentsPerPixel(
      this->GetOutputVectorLength());
}

template <typename TImageType, typename TInterpolatorPrecisionType>
unsigned int OrientedModelBaseResampleImageFilter<
    TImageType, TInterpolatorPrecisionType>::GetOutputVectorLength() {
...
template <typename TImageType, typename TInterpolatorPrecisionType>
void OrientedModelBaseResampleImageFilter<TImageType,
...
void OrientedModelBaseResampleImageFilter<TImageType,
  ...
  this->ComputeRotationParametersFromReorientationMatrix(
      orientationMatrix, parametersRotationMatrix);

  bool lastDimensionUseless = (this->GetInput(0)
                                   ->GetLargestPossibleRegion()
                                   .GetSize()[ImageDimension - 1] <= 1);
  ...
  while (!outputItr.IsAtEnd()) {
    ...
    bool isInside = this->GetInput(0)->TransformPhysicalPointToContinuousIndex(
        tmpPoint, index);
    ...
    if (!isZero(tmpRes)) {
      this->ReorientInterpolatedModel(tmpRes, parametersRotationMatrix,
                                      resRotated, threadId);
...
template <typename TImageType, typename TInterpolatorPrecisionType>
vnl_matrix<double> OrientedModelBaseResampleImageFilter<
    TImageType, TInterpolatorPrecisionType>::ComputeLinearJacobianMatrix() {
  vnl_matrix<double> reorientationMatrix(ImageDimension, ImageDimension);
  reorientationMatrix.set_identity();

  typedef itk::TranslationTransform<TInterpolatorPrecisionType, ImageDimension>
      TranslationType;
  ...
  AffineTransformType *tmpTrsf =
      dynamic_cast<AffineTransformType *>(m_Transform.GetPointer());
  ...
  typedef itk::CompositeTransform<TInterpolatorPrecisionType, ImageDimension>
      CompositeTransformType;
  CompositeTransformType *compositeTrsf =
      dynamic_cast<CompositeTransformType *>(m_Transform.GetPointer());

  for (unsigned int i = 0; i < compositeTrsf->GetNumberOfTransforms(); ++i) {
    tmpTrsf = dynamic_cast<AffineTransformType *>(
        compositeTrsf->GetNthTransform(i).GetPointer());
    if (tmpTrsf)
      reorientationMatrix *= tmpTrsf->GetMatrix().GetVnlMatrix().as_matrix();
  }
...
void OrientedModelBaseResampleImageFilter<TImageType,
                                          TInterpolatorPrecisionType>::
    ComputeLocalJacobianMatrix(InputIndexType &index,
...
void OrientedModelBaseResampleImageFilter<TImageType,
  ...
  if (m_FiniteStrainReorientation) {
    ...
    anima::ExtractRotationFromJacobianMatrix(reorientationMatrix,
                                             modelOrientationMatrix, tmpMat);
  } else
