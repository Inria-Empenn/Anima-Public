...
PyramidalSymmetryConstrainedRegistrationBridge<
    ScalarType>::PyramidalSymmetryConstrainedRegistrationBridge() {
...
void PyramidalSymmetryConstrainedRegistrationBridge<ScalarType>::Update() {
  typedef typename itk::ImageRegistrationMethod<InputImageType, InputImageType>
      RegistrationType;
  ...
  typename TransformType::ParametersType initialParams(
      TransformType::ParametersDimension);
  for (unsigned int i = 0; i < TransformType::ParametersDimension; ++i)
    initialParams[i] = 0;

  unsigned int indexAbsRefMax = 0;
  typename InputImageType::DirectionType dirRefMatrix =
      m_ReferenceImage->GetDirection();
  ...
  itk::ContinuousIndex<ScalarType, InputImageType::ImageDimension>
      centralVoxIndex;

  for (unsigned int i = 0; i < InputImageType::ImageDimension; ++i)
    centralVoxIndex[i] =
        m_ReferenceImage->GetLargestPossibleRegion().GetSize()[i] / 2.0;

  m_ReferenceImage->TransformContinuousIndexToPhysicalPoint(centralVoxIndex,
                                                            centralPoint);
  ...
  for (unsigned int i = 0; i < this->GetNumberOfPyramidLevels(); ++i) {
    ...
    for (unsigned int j = 0; j < 2; ++j) {
      lowerBounds[j + 1] = m_OutputTransform->GetParameters()[j + 1] -
                           m_TranslateUpperBound * meanSpacing;
      upperBounds[j + 1] = m_OutputTransform->GetParameters()[j + 1] +
                           m_TranslateUpperBound * meanSpacing;
    }
    ...
    case MutualInformation: {
      typedef itk::MutualInformationHistogramImageToImageMetric<InputImageType,
                                                                InputImageType>
          MetricType;
    ...
    case MeanSquares:
    default: {
      typedef itk::MeanSquaresImageToImageMetric<InputImageType, InputImageType>
          MetricType;
      typename MetricType::Pointer tmpMetric = MetricType::New();
      reg->SetMetric(tmpMetric);
      break;
    ...
    if (m_FastRegistration) {
      ...
      InputImageRegionType workRegion =
          m_ReferencePyramid->GetOutput(i)->GetLargestPossibleRegion();
      InputImageRegionType::IndexType centralIndex;
      bool isInside =
          m_ReferencePyramid->GetOutput(i)->TransformPhysicalPointToIndex(
              centralPoint, centralIndex);
  ...
  typedef itk::Matrix<ScalarType, InputImageType::ImageDimension + 1,
...
template <typename ScalarType>
void PyramidalSymmetryConstrainedRegistrationBridge<
    ScalarType>::WriteOutputs() {
  ...
  if (m_outputTransformFile != "") {
    std::cout << "Writing output transform to: " << m_outputTransformFile
              << std::endl;
...
template <typename ScalarType>
void PyramidalSymmetryConstrainedRegistrationBridge<
    ScalarType>::SetupPyramids() {
  m_InitialTransform->SetIdentity();

  typedef
      typename itk::CenteredTransformInitializer<BaseTransformType,
                                                 InputImageType, InputImageType>
  ...
  typename InputImageType::DirectionType dirRefMatrix =
      m_ReferenceImage->GetDirection();
  typename InputImageType::DirectionType dirFloMatrix =
      m_FloatingImage->GetDirection();
  double valRefMax = std::abs(dirRefMatrix(0, 0));
  ...
  MatrixType tmpMatrix =
      anima::GetRotationMatrixFromVectors(directionRefReal, directionFloReal);
  ...
  for (unsigned int i = 0; i < InputImageType::ImageDimension; ++i)
    floImageCenter[i] =
        m_FloatingImage->GetLargestPossibleRegion().GetSize()[i] / 2.0;

  typename InputImageType::PointType refCenter, floCenter;
  m_ReferenceImage->TransformContinuousIndexToPhysicalPoint(refImageCenter,
                                                            refCenter);
  m_FloatingImage->TransformContinuousIndexToPhysicalPoint(floImageCenter,
                                                           floCenter);
