...
enum Metric { MutualInformation = 0, MeanSquares };
...
public:
  ...
  using TransformType = anima::SymmetryPlaneTransform<ScalarType>;
  ...
  using Self = PyramidalSymmetryBridge;
  ...
  void ComputeRealignTransform(
      itk::Vector<double, InputImageType::ImageDimension> centralPoint,
  ...
  void SetOptimizerMaxIterations(int optMaxIterations) {
    m_optMaxIterations = optMaxIterations;
  }
  ...
  std::string GetOutputRealignTransformFile() {
    return m_outputRealignTransformFile;
  }
  void SetOutputRealignTransformFile(std::string outputRealignTransformFile) {
    m_outputRealignTransformFile = outputRealignTransformFile;
  }
