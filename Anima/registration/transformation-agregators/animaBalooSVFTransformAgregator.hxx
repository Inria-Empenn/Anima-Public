...
BalooSVFTransformAgregator<NDimensions>::BalooSVFTransformAgregator()
    : Superclass() {
...
template <unsigned int NDimensions>
bool BalooSVFTransformAgregator<NDimensions>::Update() {
...
template <unsigned int NDimensions>
void BalooSVFTransformAgregator<NDimensions>::estimateSVFFromTranslations() {
  ...
  zeroDisp.Fill(0);
  itk::ImageRegionIterator<VelocityFieldType> svfIterator(velocityField,
                                                          m_LargestRegion);
  ...
  for (unsigned int i = 0; i < nbPts; ++i) {
    if (this->GetInputTransformType() == Superclass::TRANSLATION)
      ...
      BaseMatrixTransformType *tmpTr =
          dynamic_cast<BaseMatrixTransformType *>(this->GetInputTransform(i));
      for (unsigned int j = 0; j < NDimensions; ++j)
        tmpParams[j] = tmpTr->GetOffset()[j];
    }

    double tmpWeight = this->GetInputWeight(i);
    bool isInside = weights->TransformPhysicalPointToIndex(
        this->GetInputOrigin(i), posIndexes[i]);
...
template <unsigned int NDimensions>
void BalooSVFTransformAgregator<NDimensions>::estimateSVFFromRigidTransforms() {
  const unsigned int NDegreesFreedom = NDimensions * (NDimensions + 1) / 2;
  typedef itk::Image<itk::Vector<ScalarType, NDegreesFreedom>, NDimensions>
      RigidFieldType;
  ...
  zeroDisp.Fill(0);
  itk::ImageRegionIterator<RigidFieldType> rigidIterator(rigidField,
                                                         m_LargestRegion);
  ...
  for (unsigned int i = 0; i < nbPts; ++i) {
    ...
    LogRigidTransformType *tmpTrsf =
        (LogRigidTransformType *)this->GetInputTransform(i);
    logVectors[i] = tmpTrsf->GetLogVector();
    rigidField->SetPixel(posIndexes[i], logVectors[i] * tmpWeight);
    weights->SetPixel(posIndexes[i], tmpWeight);
  }

  this->filterInputs<NDegreesFreedom>(weights, rigidField, logVectors,
                                      posIndexes);
  ...
  itk::ImageRegionIteratorWithIndex<VelocityFieldType> svfIterator(
      velocityField, m_LargestRegion);
  tmpMatrix.fill(0);
  VelocityFieldIndexType curIndex;
  VelocityFieldPixelType curDisp;

  rigidIterator =
      itk::ImageRegionIterator<RigidFieldType>(rigidField, m_LargestRegion);
  while (!svfIterator.IsAtEnd()) {
...
template <unsigned int NDimensions>
void BalooSVFTransformAgregator<
    NDimensions>::estimateSVFFromAffineTransforms() {
  ...
  zeroDisp.Fill(0);
  itk::ImageRegionIterator<AffineFieldType> affineIterator(affineField,
                                                           m_LargestRegion);
  ...
  if (dynamic_cast<anima::DirectionScaleSkewTransform<InternalScalarType> *>(
          this->GetInputTransform(0)) == 0) {
    typedef anima::MatrixLoggerFilter<InternalScalarType, ScalarType,
                                      NDimensions, NDegreesFreedom>
        MatrixLoggerFilterType;
    ...
    typedef anima::DirectionScaleSkewTransform<InternalScalarType>
        DistoTrsfType;
  ...
  this->filterInputs<NDegreesFreedom>(weights, affineField, logVectors,
                                      posIndexes);
  ...
  affineIterator =
      itk::ImageRegionIterator<AffineFieldType>(affineField, m_LargestRegion);
  while (!svfIterator.IsAtEnd()) {
...
void BalooSVFTransformAgregator<NDimensions>::filterInputs(
    WeightImageType *weights,
    typename itk::Image<itk::Vector<ScalarType, NDegreesOfFreedom>,
                        NDimensions>::Pointer &output,
    std::vector<itk::Vector<ScalarType, NDegreesOfFreedom>> &curTrsfs,
    std::vector<typename itk::Image<itk::Vector<ScalarType, NDegreesOfFreedom>,
                                    NDimensions>::IndexType> &posIndexes) {
  typedef itk::Image<itk::Vector<ScalarType, NDegreesOfFreedom>, NDimensions>
      FieldType;
  ...
  typedef anima::SmoothingRecursiveYvvGaussianImageFilter<WeightImageType,
                                                          WeightImageType>
      WeightSmootherType;
  ...
  typedef anima::SmoothingRecursiveYvvGaussianImageFilter<FieldType, FieldType>
      SVFSmoothingFilterType;
  typename SVFSmoothingFilterType::Pointer smootherPtr =
      SVFSmoothingFilterType::New();
  ...
  for (unsigned int i = 0; i < nbPts; ++i) {
    double newWeight =
        weights->GetPixel(posIndexes[i]) *
        std::exp(-residuals[i] / (averageResidual * m_OutlierRejectionSigma));
  ...
  typedef anima::BalooExternalExtrapolateImageFilter<
      ScalarType, NDegreesOfFreedom, NDimensions>
      ExtrapolateFilterType;
  typename ExtrapolateFilterType::Pointer extrapolateFilter =
      ExtrapolateFilterType::New();
