...
int main(int ac, const char **av) {
  std::string descriptionMessage =
      ...
      "generates a larger image containing the mosaic of the transformed input "
      "images.\n"
      "INRIA / IRISA - VisAGeS/Empenn Team";

  TCLAP::CmdLine cmd(descriptionMessage, ' ', ANIMA_VERSION);

  TCLAP::MultiArg<std::string> inArg(
      "i", "input", "Input image (should be used several times)", true,
      "input image", cmd);
  TCLAP::MultiArg<std::string> trArg(
      "t", "trsf", "Transformation (one linear transform for each input)",
      false, "transformation", cmd);
  TCLAP::ValueArg<std::string> outArg("o", "output", "Output mosaic image",
                                      true, "", "output mosaic image", cmd);
  TCLAP::ValueArg<std::string> outMaskArg("O", "out-mask", "Output mosaic mask",
                                          false, "", "output mosaic mask", cmd);
  TCLAP::ValueArg<std::string> geomArg("g", "geometry", "Geometry image", true,
                                       "", "geometry image", cmd);

  TCLAP::ValueArg<unsigned int> nbpArg(
      "p", "numberofthreads",
      "Number of threads to run on (default : all cores)", false,
  ...
  ImageType::Pointer geometryImage =
      anima::readImage<ImageType>(geomArg.getValue());

  using MatrixTransformType = itk::MatrixOffsetTransformBase<double, 3>;
  ...
  for (unsigned int i = 0; i < ImageType::ImageDimension; ++i) {
    lowerCornerBoundingBox[i] =
        geometryImage->GetLargestPossibleRegion().GetIndex()[i];
    upperCornerBoundingBox[i] =
        lowerCornerBoundingBox[i] +
        geometryImage->GetLargestPossibleRegion().GetSize()[i];
  }
  ...
  for (unsigned int k = 0; k < inputImages.size(); ++k) {
    ...
    ImageType::IndexType lowerCornerIndex =
        input->GetLargestPossibleRegion().GetIndex();
    ImageType::IndexType upperCornerIndex =
        lowerCornerIndex + input->GetLargestPossibleRegion().GetSize();
    ...
    if (inputTrsfs.size() != 0) {
      ...
      const itk::TransformFileReader::TransformListType *trsfList =
          reader->GetTransformList();
      itk::TransformFileReader::TransformListType::const_iterator tr_it =
          trsfList->begin();
    ...
    for (unsigned int i = 0; i < numCorners; ++i) {
      ...
      bool isInside = geometryImage->TransformPhysicalPointToContinuousIndex(
          tmpPoint, tmpRes);
  ...
  geometryImage->TransformContinuousIndexToPhysicalPoint(lowerCornerBoundingBox,
                                                         origin);
  ...
  using ResampleFilterType = anima::ResampleImageFilter<ImageType, ImageType>;
  using MaskResampleFilterType =
      anima::ResampleImageFilter<MaskImageType, ImageType>;
  ...
  using MaskImageIteratorType = itk::ImageRegionIterator<MaskImageType>;
  ...
  for (unsigned int i = 0; i < inputImages.size(); ++i) {
    ...
    MaskResampleFilterType::Pointer maskImResampler =
    ...
    MaskImageIteratorType outputMaskItr(maskImage,
                                        maskImage->GetLargestPossibleRegion());
    ImageIteratorType trsfMaskItr(maskImResampler->GetOutput(),
                                  maskImage->GetLargestPossibleRegion());
  ...
  ImageIteratorType outItr(outputImage,
                           outputImage->GetLargestPossibleRegion());
  MaskImageIteratorType maskItr(maskImage,
                                maskImage->GetLargestPossibleRegion());
