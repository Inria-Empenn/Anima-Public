...
enum Transform { Translation = 0, Rigid, Affine };

enum MetricOrientationType { None = 0, FiniteStrain, PPD };
...
public:
  using InputImageType = anima::MCMImage<double, ImageDimension>;
  ...
  using InterpolatorType =
      anima::MCMLinearInterpolateImageFunction<InputImageType>;
  using BlockMatcherType = anima::MCMBlockMatcher<InputImageType>;

  using MCModelType = anima::MultiCompartmentModel;
  using MCModelPointer = MCModelType::Pointer;

  /** SmartPointer typedef support  */
  using Self = PyramidalDenseMCMSVFMatchingBridge;
  ...
  void SetMetricOrientation(MetricOrientationType metricOr) {
    m_MetricOrientation = metricOr;
  }

  bool GetFiniteStrainImageReorientation() {
    return m_FiniteStrainImageReorientation;
  }
  void SetFiniteStrainImageReorientation(bool reor) {
    m_FiniteStrainImageReorientation = reor;
  }
