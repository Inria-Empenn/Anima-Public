...
void KissingSymmetricBMRegistrationMethod<TInputImageType>::PerformOneIteration(
    InputImageType *refImage, InputImageType *movingImage,
  ...
  this->GetAgregator()->SetInputRegions(
      this->GetBlockMatcher()->GetBlockRegions());
  this->GetAgregator()->SetInputOrigins(
      this->GetBlockMatcher()->GetBlockPositions());
  this->GetAgregator()->SetInputWeights(
      this->GetBlockMatcher()->GetBlockWeights());
  this->GetAgregator()->SetInputTransforms(
      this->GetBlockMatcher()->GetBlockTransformPointers());
  ...
  if (this->GetVerboseProgression())
    std::cout << "Matching performed in " << tmpTimeReverse.GetTotal()
              << std::endl;
  ...
  if (this->GetAgregator()->GetOutputTransformType() == AgregatorType::SVF) {
    ...
    // It's only a quarter since we are computing the half power of the
    ...
    typedef itk::MultiplyImageFilter<
        VelocityFieldType, itk::Image<double, InputImageType::ImageDimension>,
        VelocityFieldType>
        MultiplyFilterType;
    typedef itk::SubtractImageFilter<VelocityFieldType, VelocityFieldType,
                                     VelocityFieldType>
        SubtractFilterType;

    SVFTransformType *usualAddOnCast =
        dynamic_cast<SVFTransformType *>(usualAddOn.GetPointer());
    SVFTransformType *reverseAddOnCast =
        dynamic_cast<SVFTransformType *>(reverseAddOn.GetPointer());
    ...
    AffineTransformType *usualAddOnCast =
        dynamic_cast<AffineTransformType *>(usualAddOn.GetPointer());
    AffineTransformType *reverseAddOnCast =
        dynamic_cast<AffineTransformType *>(reverseAddOn.GetPointer());
...
KissingSymmetricBMRegistrationMethod<TInputImageType>::
  ...
  if (this->GetAgregator()->GetOutputTransformType() == AgregatorType::SVF) {
    ...
    DisplacementFieldTransformPointer dispTrsf =
        DisplacementFieldTransformType::New();
    SVFTransformType *svfCast = dynamic_cast<SVFTransformType *>(transform);

    anima::GetSVFExponential(
        svfCast, dispTrsf.GetPointer(), this->GetExponentiationOrder(),
        this->GetNumberOfWorkUnits(),
        (m_RegistrationPointLocation - 1) / m_RegistrationPointLocation);

    outTrsf = dispTrsf;
  } else {
    AffineTransformType *affCast =
        dynamic_cast<AffineTransformType *>(transform);
